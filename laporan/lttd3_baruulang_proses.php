...
*/
$sql="SELECT CASE WHEN htransaksi.donorbaru='0' THEN 'DS' ELSE 'DP' END AS dsdp, CASE WHEN pendonor.Jk='0' THEN 'LK' ElSE 'PR' END AS jk,
   ...
   FROM htransaksi inner join pendonor on pendonor.Kode=htransaksi.KodePendonor where (date(htransaksi.tglperiksa)>='$tanggalawal' and date(htransaksi.tglperiksa)<='$tanggalakhir') and LENGTH(htransaksi.noKantong)>1
