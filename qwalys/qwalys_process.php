...
if($op=='del'){
    ...
    if ($param =="RAE"){
        $sq_del   	= "DELETE FROM `qwalys_abs_raw` WHERE `sn`='$sn' AND `operator`='$user' AND
				   DATE(`runtime`)='$tgl' AND `parameter1`='$param' AND microplate='$plate'";
    }else{
        $sq_del       = "DELETE FROM `qwalys_abd_raw` WHERE `sn`='$sn' AND `operator`='$user' AND
                       DATE(`runtime`)='$tgl' AND `parameter1`='$param' AND microplate='$plate'";
    }
    ...
    if (($sql_delete) AND ($param =="ABD")){
		echo '<script language="javascript">';
		echo 'alert("Penghapusan data BERHASIL dilakukan.")';
		echo '</script>';
        header("Location: pmikonfirmasi.php?module=konfirm_abd");
	} else {
		echo '<script language="javascript">';
