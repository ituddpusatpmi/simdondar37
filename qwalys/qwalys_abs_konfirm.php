...
if(isset($_POST['Button']))  {
    ...
    echo "<div style ='font:16px Arial,tahoma,sans-serif;color:#ff0000'>".$time_now." (".date_default_timezone_get().")</div>";
	echo "<div style ='font:16px Arial,tahoma,sans-serif;color:#ff0000'>Jumlah sample diproses : $jumlah_sample</div>";
	if ($lanjut == 0){
        //GEN NOTRANS===========================
        ...
        $idp	= mysql_query("select abs_notrans from abs where abs_notrans like '$k_today%'
			                   order by abs_notrans DESC limit 1");
        $idp1	= mysql_fetch_assoc($idp);
        $idp2	= substr($idp1[abs_notrans],10,3);
        if ($idp2<1) {
            $idp2="000";
        }
        $int_idp2=(int)$idp2+1;
		...
		echo "<div style ='font:21px Arial,tahoma,sans-serif;color:#ff0000'>No Transaksi: <b>".$notrans."<b></div>";
		...
		for ($i=0;$i<count($_POST[kantong]);$i++) {
          ...
          if($v_aksi!=='3'){
            ...
            for($j=0;$j<$jml_kantong;$j++){
			    ...
			    //=======Audit Trial====================================================================================
			    $log_mdl ='ABS';
			    $log_aksi='Antibody Screening : Qwalys 3, No.Transaksi: '.$notrans.'; Kantong No:'.$no_ktg.', hasil : '.$v_hasil;
			    include('user_log.php');
			    //======================================================================================================
                if ($v_hasil=='Pos'){
                    $qupd_ktg=mysql_query("update stokkantong set abs='$v_hasil', status='7'  where noKantong='$no_ktg'");
                } else {
            ...
            if ($qupd_raw){
            //echo "Confirmed id $v_id<br>";
            }else{
                echo "Confirmed Gagal id $v_id<br>";
            }
          } else {
              echo " sample $v_sample : ditunda<br>";
          }
        }
        echo "<div style ='font:16px Arial,tahoma,sans-serif;color:#ff0000'>Tunggu.....</div>";
        echo "<meta http-equiv='refresh' content='1;url=pmikonfirmasi.php?module=qwalys_view_confirm&notrans=$notrans&mode=1'>";
...
<table border=0 cellpadding="5" cellspacing="5" width="100%">
   <tr>
		<td align="left" style="background-color: #ffffff;font-size:24px; color:#0099ff;text-shadow: 1px 1px 1px #000000; font-family:Verdana;"><b>Konfirmasi Hasil Pemeriksaan Antibody Screening Qwalys<sup>&reg</sup> 3</td>
        ...
        <td colspan="10" rowspan="3" align="left" style="background-color: ghostwhite; color: #000000">
            <b>Catatan :</b>
                <ol>
                    ...
                    <li>Apabila kantong dicekal, pendonor otomatis dicekal dengan kode ABS POS</li>
