...
<body>
	<font size="4" color=00008B>DATA PEMERIKSAAN ALINITY I YANG BELUM TERKONFIRMASI</b></font><br><br>
	<style>
	...
	while($tmp=mysql_fetch_assoc($qraw)){
		...
		<tr style="font-size:13px; color:#000000; font-family:Verdana;" onMouseOver="this.className='highlight'" onMouseOut="this.className='normal'">
			...
			<td>
				<a href="pmiimltd.php?module=alinitykonfirmasi&ins=<?=$tmp['instrument_name']?>&tgl=<?=$tmp['tanggal']?>&usr=<?=$tmp['operator']?>" class="swn_kiri">Konfirmasi</a> |
				<a href="pmiimltd.php?module=alinityprocess&op=del&ins=<?=$tmp['instrument_name']?>&tgl=<?=$tmp['tanggal']?>&usr=<?=$tmp['operator']?>" onclick="return confirm('PERHATIAN \n \nYakin akan menghapus data transfer alat \n<?=$tmp['instrument_name']?>, \nTanggal: <?=$tmp['tanggal']?>, \nOperator: <?=$tmp['operator']?>, \nJumlah tes: <?=$tmp['jml_sample']?> ?');" class="swn_kanan">Hapus</a>
				
		</tr>
	<?}
	if ($no==0){?>
		<tr class="record">
			<td colspan=8> .:: Semua Hasil Pemeriksaan Telah Dikonfirmasi ::.</td>
	<?}?>
	</table><br>
	<a href="pmiimltd.php?module=import_alinity"class="swn_button_blue">Kembali</a>
	<?
?>
</body>
</html>
