			...
			if ($aksi_konfirm!=="3"){
                ...
                if ($idraw_b!=="0"){
                    $sq_hbsag="insert into pmi.hasilelisa (noKantong,OD,COV,Hasil,notrans,jenisPeriksa,tglPeriksa,
						      ...
						      '$user_instrument','$ptgKonfirmasi','$ptgSah','$lotr_b','chlia','$umur', '$jnsdonor', '$barulama', '$catatan')";
                ...
                if ($idraw_c!=="0"){
                    $sq_hcv  ="insert into pmi.hasilelisa (noKantong,OD,COV,Hasil,notrans,jenisPeriksa,tglPeriksa,
						      ...
						      '$user_instrument','$ptgKonfirmasi','$ptgSah','$lotr_c','chlia','$umur', '$jnsdonor', '$barulama', '$catatan')";
                    echo "- EIA C - ";
                ...
                if ($idraw_i!=="0"){
                    $sq_hiv  ="insert into pmi.hasilelisa (noKantong,OD,COV,Hasil,notrans,jenisPeriksa,tglPeriksa,
						      ...
						      '$user_instrument','$ptgKonfirmasi','$ptgSah','$lotr_i','chlia','$umur', '$jnsdonor', '$barulama', '$catatan')";
                    echo "- EIA I - ";
                ...
                if ($idraw_s!=="0"){
                    $sq_syp  ="insert into pmi.hasilelisa (noKantong,OD,COV,Hasil,notrans,jenisPeriksa,tglPeriksa,
						      ...
						      '$user_instrument','$ptgKonfirmasi','$ptgSah','$lotr_s','chlia','$umur', '$jnsdonor', '$barulama', '$catatan')";
                    echo "- EIA S - ";
                ...
                //------------------------------------------------------------------------------------------------------
                $sq_setstatus="UPDATE lis_pmi.`cobas6000` SET `confirm`='1'
...
$Sq=mysql_query("SELECT `id`, `instrument_name`, `run_time`, `sample_id`, `parameter_name`, `parameter_no`, 
				...
				`on_insert`, `comment`, `confirm`, `confirm_time`, `user_confirm` FROM `cobas6000` 
...
<table border=0 cellpadding="5" cellspacing="5" width="100%">
   <tr>
		<td align="left" style="background-color: #ffffff"><font size=5 color="blue"><b>KONFIRMASI HASIL PEMERIKSAAN COBAS 6000.1</b></font></td>
...
<form name="manual_input" align="left" method="post" action="<?echo $PHPSELF?>">
	...
	<tr class="field">
		...
		<select name="reagen1" id="reagen1" onChange="show1(1)">
				<option value="-">-</option>
				<? 
				$jreagen1=mysql_query("select * from reagen where Nama like '%Roche%Hbsag%' and aktif='1' and jumTest>0",$con_pmi);
		...
		<select name="reagen2" id="reagen2" onChange="show2(2)">
				<option value="-">-</option>
				<? 
				$jreagen1=mysql_query("select * from reagen where Nama like '%Roche%hcv%' and aktif='1' and jumTest>0",$con_pmi);
				while ($jreagen11=mysql_fetch_assoc($jreagen1)) { ?>
					<option value="<?=$jreagen11[Nama]?>*<?=$jreagen11[noLot]?>*<?=$jreagen11[kode]?>*<?=$jreagen11[jumTest]?>*<?=$jreagen11[tglKad]?>">
								   <?=$jreagen11[Nama]?>-<?=$jreagen11[noLot]?>-<?=$jreagen11[jumTest]?> T
					</option><?
		...
		<select name="reagen3" id="reagen3" onChange="show3(3)">
				<option value="-">-</option>
				<? 
				$jreagen1=mysql_query("select * from reagen where Nama like '%Roche%HIV%' and aktif='1' and jumTest>0", $con_pmi);
				while ($jreagen11=mysql_fetch_assoc($jreagen1)) { ?>
					<option value="<?=$jreagen11[Nama]?>*<?=$jreagen11[noLot]?>*<?=$jreagen11[kode]?>*<?=$jreagen11[jumTest]?>*<?=$jreagen11[tglKad]?>">
								   <?=$jreagen11[Nama]?>-<?=$jreagen11[noLot]?>-<?=$jreagen11[jumTest]?> T
					</option><?
		...
		<select name="reagen4" id="reagen4" onChange="show4(4)">
				<option value="-">-</option>
				<? 
				$jreagen1=mysql_query("select * from reagen where Nama like '%Roche%Syphilis%' and aktif='1' and jumTest>0",$con_pmi);
				while ($jreagen11=mysql_fetch_assoc($jreagen1)) { ?>
					<option value="<?=$jreagen11[Nama]?>*<?=$jreagen11[noLot]?>*<?=$jreagen11[kode]?>*<?=$jreagen11[jumTest]?>*<?=$jreagen11[tglKad]?>">
									<?=$jreagen11[Nama]?>-<?=$jreagen11[noLot]?>-<?=$jreagen11[jumTest]?> T
					</option><?
    ...
    <? mysql_select_db("$db_pmi",$con_pmi); ?>
	<tr class="field">
		<td colspan="2" align="left">Operator Cobas</td>
	...
	<a href="pmiimltd.php?module=rochelistkonfirmasi"class="swn_button_blue">Kembali ke list data</a>
	<a href="pmiimltd.php?module=import_roche"class="swn_button_blue">Kembali ke Awal</a>
    <?
    if ($no!==0){?>
