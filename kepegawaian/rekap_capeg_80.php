...
<?$trans0=mysql_query("select * from pegawai where CAST(tmt80 as date)>='$today' and CAST(tmt80 as date)<='$today1' and statuspeg='1' order by tmt80 ASC");?>
<h2 class="table">Jumlah Pengajuan KGB yang sudah waktunya dari Tgl <?='&nbsp'.'&nbsp'.'&nbsp'?>:<?='&nbsp'.'&nbsp'.'&nbsp'?><?=$pertgl?> - <?=$perbln?> - <?=$perthn?> <?='&nbsp'.'&nbsp'.'&nbsp'?>s/d<?='&nbsp'.'&nbsp'.'&nbsp'?> <?=$pertgl1?> - <?=$perbln1?> - <?=$perthn1?><?='&nbsp'.'&nbsp'.'&nbsp'?>Berjumlah<?='&nbsp'.'&nbsp'.'&nbsp'?>:<?='&nbsp'.'&nbsp'.'&nbsp'?><?=mysql_num_rows($trans0)?>  Data</h2>
...
<br>
<form name=xls method=post action=kepegawaian/rekap_kgb_xls.php>
...
<input type=submit name=submit2 value='Print Rekap Pengajuan KGB (.XLS)'>
</form>
</table>
