...
include ("/var/www/simudda/config/koneksi.php");
    ...
    $donor      = date("Y-m-d");
    $dk         = mysql_query("select nik,tglsk,nosk,gajilama,gajibaru,tglberlaku,tglawalrapel,tglakhirrapel,jmlbulanrapel,nilairapel,jmlrapel,hp from kppeg where status='2' and tanggal_entry='$donor'  and length(hp)>9 ");
	...
	if ($kgb1[Jk]=='1' and $kgb1[Status]=='0') $sapa='Sdri';
        $telp   =$dk1[hp];
        $pesan  ='Yth. '.$sapa.'. '.$kgb1[Nama].',KP anda sudah selesai diproses dengan data no SK :'.$dk1[nosk].', berlaku mulai tgl: '.$dk1[tglberlaku].',rapel anda berjumlah:'.$dk1[jmlbulanrapel].' bulan, dari tgl:'.$dk1[tglawalrapel].',s/d :'.$dk1[tglakhirrapel].' dengan nilai rapel/bulan :Rp.'.$dk1[nilairapel].', nilai total rapel anda Rp. '.$dk1[jmlrapel].'. terima kasih';
