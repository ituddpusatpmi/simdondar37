...
if (isset($_POST[submit])) {
	...
	$donorlagi 	= mktime(0,0,0,date("m")+3,date("d"),date("Y"));	$bagian	= $_POST["bagian"];
	$donorlagi 	= mktime(0,0,0,date("m")+3,date("d"),date("Y"));	$bagian1= $_POST["bagian1"];
	...
	$direktorat	= $_POST["direktorat"];					$kelompok	= $_POST["kelompok"];
	 ...
	 $tmt1 = str_replace("-","",$tglkontrak);
	 $tmt1 = str_replace(" ","",$tmt1);
	 $tmt1 = str_replace(",","",$tmt1);

	 $nm=strtoupper(substr($nama1,0,8));
	 $tm=strtoupper(substr($tmt1,2,4));
	 $idp	= mysql_query("select id,id1 from tempat_donor where active='1'",$con);
	 $idp1	= mysql_fetch_assoc($idp);

	if ($jk=='0')$jk1='1';
	if ($jk=='1')$jk1='2';
	 $kdtp	= 'K'.$utd[id].$nm.$tm.$jk1.'1';
	 $idp	= mysql_query("select Kode from pegawai_kontrak  
	...
	$sekarang = date("Y-m-d h:m:s");
	 ...
	 $tambah_sql="insert into pegawai_kontrak 
					...
					`tanggal_entry`,`ijasah`,`statuspeg`,`golongan`,`jabatan`,`tmtkontrak`,`bagian`,`bagian1`,`direktorat`)
			   values ('$kode','$noktp','$nama','$alamat','$jk','$telp','$tptlhr','$tgllhr','$status','$goldarah',
					'$rhesus','$kelurahan','$kecamatan','$wilayah','$kodepos','$title','$telp2','$ibukandung','$namauser','$sekarang',
					'$sekarang','$ijasah','$statuspeg','$golongan','$jabatan','$tglkontrak','$bagian','$bagian1','$direktorat')";
	...
	$cek_tmpudd=1;
	$cek_tmpudd=mysql_num_rows(mysql_query("Select * from tempudd where modul='KEPEG'"));
	if ($cek_tmpudd==0) {
	   $tambah_tmp=mysql_query("INSERT INTO tempudd (modul,alamat,kelurahan,kecamatan,wilayah,kodepos,tptlahir)
				   values ('KEPEG','$alamat','$kelurahan','$kecamatan','$wilayah','$kodepos','$tptlhr')");
	} else {
	   $tambah_tmp=mysql_query("UPDATE tempudd  SET alamat='$alamat',kelurahan='$kelurahan', kecamatan='$kecamatan',wilayah='$wilayah',
				   kodepos='$kodepos', tptlahir='$tptlhr'
				   where modul='KEPEG'");
	}
	...
	if ($tambah) {
			...
			mysql_query("UPDATE pegawai_kontrak set umur=(TO_DAYS(NOW())- TO_DAYS(TglLhr)) / 365.25");
			
			//mysql_query("UPDATE pegawai set sisacuti='0' where tglcuti < '$today'");
			mysql_query("update pegawai_kontrak set tmt80=(tmtkontrak + interval 1 year) where statuspeg='0'");
			mysql_query("UPDATE pegawai_kontrak set masakerja=(TO_DAYS(NOW())- TO_DAYS(tmtkontrak)) / 365.25");
		 ...
		 switch ($lv0){
			   case "kepegawaian":
					?>
				<META http-equiv="refresh" content="0; url=pmikepegawaian.php?module=search_kepeg_kontrak">
					<?
			   break;
			  default:
...
switch ($lv0) {
	 case "kepegawaian":
		  ?><form name="reg" autocomplete="off" method="post" action="pmikepegawaian.php?module=registrasi_kontrak"><?
	 break;
        case "admin":
		  ?><form name="reg" method="post" action="pmiadmin.php?module=registrasi_kepeg_kontrak"><?
	 break;
	 default:
...
<h1 class="table">FORM INPUT DATA KARYAWAN KONTRAK||
<input type="button" value="SEARCH DATA KARYAWAN KONTRAK" onClick="document.location.href='pmikepegawaian.php?module=search_kepeg_kontrak';"></h1>
<? } else if ($_SESSION[leveluser]=='kepegawaian') { ?>
<h1 class="table">FORM INPUT DATA KARYAWAN KONTRAK||
<input type="button" value="SEARCH DATA KARYAWAN KONTRAK" onClick="document.location.href='pmikepegawaian.php?module=search_kepeg_kontrak';"></h1>
<? } else { ?>
<h1 class="table">FORM INPUT DATA KARYAWAN KONTRAK||
<input type="button" value="SEARCH DATA KARYAWAN KONTRAK" onClick="document.location.href='pmikasir.php?module=search_kepeg_kontrak';"></h1>
<? } ?>
				...
				<tr>
			     ...
			     <td class="input">
						 <select name="kelompok">
							  <option value="0" selected>Non Teknis</option>
			   ...
			   <tr> 
			     ...
			     <td class="input">
						 <select name="statuspeg">
							  <option value="0" selected>Kontrak</option>
							  <option value="1">Tetap 80%</option>
