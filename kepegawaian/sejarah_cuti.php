		<?
		$data31=mysql_query("select * from pegawai where Kode='$_GET[kode]'");
...
<?
$data=mysql_query("select * from cutipeg where nik='$data3[Kode]'");
?>
<table>
<h3 class="list">Jumlah Proses Cuti :  <?=mysql_num_rows($data)?>  Kali</h3>
</table>
<!--?
...
<table class="list" cellpadding=5 cellspacing=1>
	<tr class="field">
		...
		<td colspan='2'>Tanggal Cuti</td>
		<td rowspan='2'>Lama Cuti</td>
	...
	?>
		...
		<td><?=$data1[notrans]?></td>
		<?
		if ($data1[jenis]=='0') $stat="Tahunan";
		if ($data1[jenis]=='1') $stat="Melahirkan";
		if ($data1[jenis]=='2') $stat="Ibadah Haji";
		if ($data1[jenis]=='3') $stat="Cuti Besar";
		?>
		<td> <?=$stat?></td>
		<td><?=$data1[tglawal]?></td>
		<td><?=$data1[tglakhir]?></td>
		<td><?=$data1[lamacuti]?> Hari</td>
...
<br>
<form name=xls method=post action=kepegawaian/sejarah_cuti_xls.php>
