...
if (isset($_POST[submit])) {
	...
	$kelompok	= $_POST["kelompok"];					$bagian1	= $_POST["bagian1"];
	...
	$tambah=mysql_query("UPDATE pegawai_kontrak SET 
		  ...
		  golongan='$golongan', ijasah='$ijasah', golongan='$_POST[golongan]', jabatan='$jabatan',statuspeg='$statuspeg',tmtkontrak='$tmt',
		  bagian='$bagian1',direktorat='$direktorat' where Kode='$kode'");
	...
	switch ($lv0){
		case "kepegawaian":
			?><META http-equiv="refresh" content="1; url=pmikepegawaian.php?module=search_kepeg_kontrak"><?
...
if (isset($_GET[Kode])) {
	 $perintah=mysql_query("select * from pegawai_kontrak where Kode='$_GET[Kode]'");
	 $nrow=0;
	 if ($perintah) {
     ...
     <td>Status Pegawai</td>
			...
			?>
			 <select name="statuspeg">
							  <option value="0" <?=$A3?> >Kontrak</option>
							  <option value="1" <?=$B3?> >Capeg 80%</option>
...
<tr>
	...
	<td>Jenis Bagian</td>
					<td class="input">
						 <select name="bagian1" >
								   <?php
								   $q1="select * from bagianpeg";
								   $do1=mysql_query($q1,$con);
										$select1="";
								   while($data1=mysql_fetch_assoc($do1)){
									if ($data1[kode]==$row[bagian]) $select1='selected';
								   ?>
							  <option value="<?=$data1[kode]?>"<?=$select1?>>
								   <?=$data1[nama]?>
							  </option>
								   <?
										$select1="";
...
<td class="input">
			...
			<? 
			$K0='';$K1='';$K2='';
			if ($row[kelompok]=='0') 	   $K0='selected';
			if ($row[kelompok]=='1') 	   $K1='selected';
			if ($row[kelompok]=='2') 	   $K2='selected';
			
			
			?>
			 <select name="kelompok">
							  <option value="0" <?=$K0?> >Non Teknis</option>
							  <option value="1" <?=$K1?> >Teknis</option>
							  <option value="2" <?=$K2?> >Managerial</option>
							  </select>
	 </td>
...
<td class="input">
						 <select name="direktorat" >
								   <?php
								   $q2="select * from direktoratpeg";
								   $do2=mysql_query($q2,$con);
										$select2="";
								   while($data2=mysql_fetch_assoc($do2)){
									if ($data2[kode]==$row[direktorat]) $select2='selected';
								   ?>
							  <option value="<?=$data2[kode]?>"<?=$select2?>>
								   <?=$data2[nama]?>
							  </option>
								   <?
										$select2="";
	...
	<td>TMT KONTRAK</td>
		<td>
	<input type="text" name="tmt" id="datepicker1" value="<?=$row[tmtkontrak]?>" size=10>
		</td>
