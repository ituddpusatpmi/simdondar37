...
$tgl	= date("d");
$kdtp	= "KP".$tgl.$bl.$th."-";
$idp	= mysql_query("select notrans from kppeg where notrans like '$kdtp%' order by notrans DESC");
$idp1	= mysql_fetch_assoc($idp);
...
if (isset($_POST['submit'])){
	...
	$tambah=mysql_query("insert into kppeg 
			(notrans,nik,tglajukan,Status,tglsk,nosk,gollama,golbaru,tanggal_entry,
			 tanggal_update,ijasah,jabatan,pencatat,up,masakerja)
	value ('$id_transaksi_baru','$nik','$tglawal','$_POST[status]','$tglakhir','$_POST[nosk]','$_POST[golonganlama]','$_POST[golonganbaru]','$today1','$today1','$_POST[ijasah]','$_POST[jabatan]','$namauser','1','$masakerja')");
		
			
	if ($tambah) {
		echo "Data Telah berhasil dimasukkan<br>";
					
		mysql_query("UPDATE kppeg set lamaproses=(TO_DAYS(tglsk)- TO_DAYS(tglajukan))");		
			...
			$up0=mysql_query(" update pegawai as p,kppeg as k set p.kp=k.tglsk,p.golongan=k.golbaru,p.kp1=(k.tglsk + interval 4 year) where p.Kode=k.nik and k.up='1'");
	...
	switch ($lv0){
		case "kepegawaian":
			?><META http-equiv="refresh" content="1; url=pmikepegawaian.php?module=laporan_kepeg"><?
...
<h1 class="table">FORM INPUT DATA KP KARYAWAN</h1>
