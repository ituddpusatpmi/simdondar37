...
if (isset($_POST['submit'])){
	...
	$tambah=mysql_query("insert into htransaksi 
			...
			JenisDonor,id_permintaan,Status,Nopol,apheresis,kendaraan,shift,kota) 
	value ('$id_transaksi_baru','$id','$id1','$today1','-','9','$tmpt[0]','$instansifix',
		'$_POST[JenisDonor]','$_POST[id_permintaan]','0','$nopol','$apheresisfix','$kendaraanfix','$shift','$kota[id]')");
	...
	switch ($lv0){
		...
		break;
		case "kasir":
			?><META http-equiv="refresh" content="1; url=pmikasir.php?module=checkup&NoTrans=<?=$id_transaksi_baru?>"><?
		break;
		case "aftap":
			?><META http-equiv="refresh" content="1; url=pmiaftap.php?module=checkup&NoTrans=<?=$id_transaksi_baru?>"><?
		break;
		case "bdrs":
...
<table class="form" cellspacing="0" cellpadding="2">
	...
	<tr>
		...
		<td class="input">
			<?
			$rs1=mysql_fetch_assoc(mysql_query("select * from detailinstansi where aktif='1'"));
		echo $rs1[nama];
		?>
		...
		$kendaraan1=mysql_fetch_assoc(mysql_query("select k.kendaraan, d.KodeDetail,d.aktif from kegiatan as k, detailinstansi as d where k.kodeinstansi=d.KodeDetail and d.aktif='1'"));
