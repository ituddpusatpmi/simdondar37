...
if (isset($_POST['simpan2'])) {
	...
	if ( (!empty($_POST['sample'])) and (!empty($_POST['od'])) ){
		...
		$udd	    = mysqli_query($dbi,"SELECT id,id1 from tempat_donor where active='1'");
    	$idp1	    = mysqli_fetch_assoc($udd);
    	$tempat	    = $idp1['id1'];
    	$log_mdl    = strtoupper($_SESSION['leveluser']);
    	$namauser   = $_SESSION['namauser'];
    	$client_ip  = $_SERVER['REMOTE_ADDR'];
    	$time_aksi  = date("Y-m-d H:i:s");
		$log_aksi='Pemeriksaan NAT : '.$v_reagen.', Kantong no:'.$v_kantong;
        ...
        // Update kantong
		$prefix_no_kantong = preg_replace('/[A-Z]+$/', '', $v_kantong);
		$sl_updkantong = "UPDATE `stokkantong`  SET `hasilNAT`='v_hasil', `tgl_abs`='$v_tgl'   WHERE `noKantong` REGEXP '^" . $prefix_no_kantong . "[A-Z]$'";
					...
					<div class="col-sm-12">
						<div class="panel panel-primary">
							<div class="panel-heading w3-theme-d5">
									<h3 class="panel-title">Pemeriksaan NAT </h3>
							</div>
	...
	<script>
        $(document).ready(function(){
			$('#tanggal').datetimepicker({
				format: 'YYYY-MM-DD HH:mm',  // Format dengan tanggal, jam, dan menit
				icons: {
