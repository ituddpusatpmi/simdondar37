...
?>
<h1>PENCARIAN DONOR APHERESIS</h1>
<?
...
$jd=mysql_query("select * from pendonor where 
		 ...
		 COALESCE (Alamat,'') like '%$srcalamat%' and 			 COALESCE (kelurahan,'') like '%$srckelurahan%' and 			 COALESCE (kecamatan,'') like '%$srckecamatan%' and 			 COALESCE (wilayah,'') like '%$srcwilayah%' and 		 COALESCE (TempatLhr,'') like '%$srctmptlhr%' and 			 COALESCE (TglLhr,'') like '%$srctgllahir%' and 		 COALESCE (Rhesus,'') like '%$srcrhesus%' and 			 COALESCE (GolDarah,'') = '$src_abo' and 
		 COALESCE (telp2,'') like '%$srctelp%' and 
		 COALESCE (apheresis,'') = '1' ORDER BY 		 tglkembali_apheresis ASC");
...
<th align="center">Tgl Kembali Apheresis</th>
</tr>
...
echo $statusc ?></td>
<td align="center"><?=$datadonor['tglkembali_apheresis'] ?></td>
</tr>
<?
$no++;
} ;}?>
