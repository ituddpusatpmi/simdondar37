...
$sq_elisa=mysql_query("SELECT `id`, `noKantong`, `OD`, `COV`, `notrans`,
                          ...
                          FROM `hasilelisa` WHERE `noKantong`='$nkt' order by `id`");                       
?>
<br>DATA PEMERIKSAAN IMLTD ELISA & CHLIA<br>
<table class="list" border=1 cellpadding="2" cellspacing="2" width="100%" style="border-collapse:collapse">
    <tr style="background-color:#FF6346; font-size:14px; color:#FFFFFF; font-family:Verdana;" onMouseOver="this.className='highlight'" onMouseOut="this.className='normal'">
        <th rowspan="2">ID</th>
    ...
    if ($no=="0"){
        ?><tr style="font-size:13px; color:#000000; font-family:Verdana;" onMouseOver="this.className='highlight';" onMouseOut="this.className='normal';">
            <td colspan="13" class=input align="center">TIDAK ADA DATA PEMERIKSAAN IMLTD ELISA & CHLIA</td>
        </tr>
    <?}
    ?>
</table>
...
//DATA PEMERIKSAAN IMLTD ELISA ==========================================================================================
$sq_elisa=mysql_query("SELECT `noKantong`, `OD`, `COV`, `notrans`,
                          case
                            when `jenisPeriksa`='0' then 'HBsAg'
                            when `jenisPeriksa`='1' then 'Anti HCV'
                            when `jenisPeriksa`='2' then 'Anti HIV'
                          ...
                          case
                            ...
                            when `Hasil`='1' then 'Reaktif' 
                            when `Hasil`='2' then 'Grayzone'  End As Hasil,
                          `tglPeriksa`, `dicatatOleh`, `dicekOleh`, `DisahkanOleh`, `noLot`, `Metode`,`instrument` 
                          FROM `hasilclia` WHERE `noKantong`='$nkt'");                       
?>
...
<?
/*$sql_arc="SELECT `id`, `no_trans`, `instr`, `instr_v`, `scc_serial`, `interface_sn`, `arc_serial`, date(`trans_time`) as trans_time, `user`,
            ...
            FROM `imltd_arc_konfirm` WHERE `id_tes`='$nkt'";
$qry_arc=mysql_query($sql_arc);*/
?>
<!--br>DATA PEMERIKSAAN DENGAN ABBOT ARCHITECT <i>i</i>2000SR<br>
<table class="list" border=1 cellpadding="2" cellspacing="2" width="100%" style="border-collapse:collapse">
    <tr style="background-color:#FF6346; font-size:14px; color:#FFFFFF; font-family:Verdana;" onMouseOver="this.className='highlight'" onMouseOut="this.className='normal'">
        <th rowspan="2">ID</th>
        ...
        <th rowspan="2">Operator<br>Architec</th>
   ...
   /* $no="0";
    while ($arc=mysql_fetch_assoc($qry_arc)){ $no++;
        ...
        */?>
        <!--tr style="font-size:13px; color:#000000; font-family:Verdana;" onMouseOver="this.className='highlight';" onMouseOut="this.className='normal';">
            <td class=input><a href="pmiimltd.php?module=imltd_arc2000_cetakhasil_arc&noid=<?=$arc['id']?>&notrans=<?=$arc['no_trans']?>" target="blank"><?=$arc['no_trans']?></a></td>
            
            <td class=input><?=$arc['instr'].' '.$arc['arc_serial']?></td>
    ...
    if ($no=="0"){
        ?><tr style="font-size:13px; color:#000000; font-family:Verdana;" onMouseOver="this.className='highlight';" onMouseOut="this.className='normal';">
            <td colspan="29" class=input align="center">TIDAK ADA DATA PEMERIKSAAN ARCHITECH</td>
        </tr>
    <?}
    */?>
...
<!--/table-->
                      ...
                      $sq_nat=mysql_query("SELECT *,
                                                ...
                                                FROM `hasilnat` WHERE `noKantong` = '$nkt' order by `natid`");

                      ?>
                    <br>DATA PEMERIKSAAN DENGAN METODE NAT <i>i</i>2000SR<br>
                      <table class="list" border=1 cellpadding="2" cellspacing="2" width="100%" style="border-collapse:collapse">
                                          <tr style="background-color:#FF6346; font-size:14px; color:#FFFFFF; font-family:Verdana;" onMouseOver="this.className='highlight'" onMouseOut="this.className='normal'">
                              <th rowspan="2">ID</th>
