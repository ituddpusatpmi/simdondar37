...
<tr><td>Kode Pendonor</td><td>:</td><td><input type=text name='kode' size='20' placeholder="ID KARTU DONOR" onchange='disabletext(this.value);'></td></tr>
<tr><td>No. KTP/Identitas</td><td>:</td><td><input type=text name='NoKTP' size='20' placeholder="No. KTP/Identitas"></td><td>Tempat Lahir</td><td>:</td><td><input
...
?>
<!-- Antri Donor -->
<a href="pmi<?echo $_SESSION[leveluser] ?>.php?module=transaksi_donorlk&Kode=<? echo $datadonor['Kode'] ?>&apheresis=0" target="isiadmin" class="fisheyeItem"><img src="../images/bloodbag.png" width=25 height=15 /></a> <?} else ?>
...
?>
<h1>PENCARIAN DONOR LUAR KOTA</h1>
<?
...
<tr><td>Kode Pendonor</td><td>:</td><td><input type=text name='kode' size='20' placeholder="ID KARTU DONOR"></td><td>Asal UTD </td><td>:</td><td><input type='text' name="udd" id='instansi' placeholder="Ketikkan Nama UTD" required></td></tr>
...
//-----------DATABASE NASIONAL----------
$jd=mysql_query("select * from pendonor where pkode like '%$srckode%' and pnoktp like '%$srcNoKTP%' and pnama like '%$srcnama%' and palamat like '%$srcalamat%' and pkelurahan like '%$srckelurahan%' and pkecamatan like '%$srckecamatan%' and pwilayah like '%$srcwilayah%' and ptempatlahir like '%$srctmptlhr%'  and ptglLahir like '%$srctgllahir%' and pgoldarah like '%$src_abo%' and prhesus like '%$srcrhesus%'  and ptelp2 like '%$srctelp%' and putd like '%$srcudd%' order by pnama asc");
...
<th align="center">Asal UTD</th>

</tr>

<?  
while ($datadonor=mysql_fetch_array($jd)){
$kode=$datadonor['pkode'];
$today=date('Y-m-d');
if ($datadonor['pcekal']=='1') 
echo "<tr style=background-color:#FF6346>";
if ($datadonor['pcekal']=='2') 
...
if ($datadonor[pjk]=='1'){
$tahun=date('Y');
$jumtransaksiperempuan=mysql_query("select * from htransaksi where htkodependonor='$kode' and year(httgl)='$tahun'");
if (date('Y-m-d')>=$datadonor['ptglkembali'] and ($datadonor['pcekal']=='0') and (mysql_num_rows($jumtransaksiperempuan) <'4') ){

//TRANSAKSI BELUM DIBUAT

?>
<!--a href="pmi<?echo $_SESSION[leveluser] ?>.php?module=transaksi_donor&Kode=<? echo $datadonor['pkode'] ?>&apheresis=0" target="isiadmin" class="fisheyeItem"><img src="../images/bloodbag.png" width=25 height=15 /></a-->
<img src="../images/bloodbag.png" width=25 height=15 /> <?}} else
//antri Donor Laki-Laki
if ($datadonor[pjk]=='0'){
if (date('Y-m-d')>=$datadonor['ptglkembali'] and ($datadonor['pcekal']=='0') ){
?>
<!-- Antri Donor -->
<!--a href="pmi<?echo $_SESSION[leveluser] ?>.php?module=transaksi_donor&Kode=<? echo $datadonor['pkode'] ?>&apheresis=0" target="isiadmin" class="fisheyeItem"><img src="../images/bloodbag.png" width=25 height=15 /></a-->
<img src="../images/bloodbag.png" width=25 height=15 /> <?} } else ?>
...
<!--Edit Data Donor -->
<a href="pmi<?echo $_SESSION[leveluser] ?>.php?module=eregistrasiluar&Kode=<? echo $datadonor['pkode'] ?>" target="isiadmin" class="fisheyeItem"><img src="../images/ubah.png" width=25 height=15 /></a>
<!-- Apheresis -->
<?
if (date('Y-m-d')>=$datadonor['tglkembali_apheresis'] and ($datadonor['Cekal']=='0' ) and ($datadonor['apheresis']=='1')  ){
?>
<a href="pmi<?echo $_SESSION[leveluser] ?>.php?module=transaksi_donor&Kode=<? echo $datadonor['pkode'] ?>&apheresis=1"><img src="../images/aferesis.png" width=25 height=15></a> 
<? };?>

<br>
<a href="pmi<?echo $_SESSION[leveluser] ?>.php?module=history_luar&q=<? echo $datadonor['pkode']?>&utd=<? echo $srcudd?>" target="isiadmin" class="fisheyeItem"><?=$datadonor['pkode'] ?></a></td>
<td align="center"><?=$datadonor['pnama'] ?></td>
<td align="center"><?=$datadonor['palamat'] ?></td>
<td align="center"><?=$datadonor['pkelurahan'] ?></td>
<td align="center"><?=$datadonor['pkecamatan'] ?></td>
<td align="center"><?=$datadonor['pwilayah'] ?></td>
<td align="center"><?=$datadonor['pgoldarah'] ?></td>
<td align="center"><?=$datadonor['prhesus'] ?></td>
<td align="center"><?=$datadonor['papheresis'] ?></td>
<?
if ($datadonor['pjk']=='0')
$jeniskelamin='Laki-Laki';
if ($datadonor['pjk']=='1')
...
?>
<td align="center"><?=$jeniskelamin?></td>
<td align="center"><?=$datadonor['ptempatlahir'] ?></td>
<td align="center">
<? $tgllahir=date("d/m/Y",strtotime($datadonor['ptgllahir'])); 
echo $tgllahir;
?>
</td>
<td align="center"><?=$datadonor['ptelp2'] ?></td>
<td align="center"><?=$datadonor['ptelp'] ?></td>
<td align="center"><?=$datadonor['pjmldonor'] ?> kali</td>
<td align="center">
<?
if ($datadonor['pcekal']=='0') 
$statusc='OK';
...
<? 
$putd=$datadonor['putd'];
$asal=mysql_fetch_assoc(mysql_query("select nama from utd where id='$putd'"));
?>
<td align="center"><?=$asal['nama'] ?></td>
<!--td align="center"><?=$datadonor['pcetak']="<a href=pmi".$_SESSION[leveluser].".php?module=historycetak&kode=".$kode." TITLE=\"DETIL\">
												".$datadonor['pcetak']." kali</a> "?></td-->
</tr>
<?
$no++;
} ;}?>
