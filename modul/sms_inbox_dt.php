...
$array_bulan = array(1=>'Jan','Feb','Mar', 'Apr', 'Mei', 'Jun','Jul','Ags','Sep','Okt', 'Nov','Des');
	 ...
	 */
	$aColumns = array( 'ReceivingDateTime', 'SenderNumber', 'TextDecoded' ,'ID');
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			else if ( $aColumns[$i] != ' ' )
			{
				if($i==0){
					...
					$id=$aRow[ $aColumns[$i]];
					$row[] =$link. "<a href=pmip2d2s.php?module=balas_sms&ID=".$id." TITLE=\"Balas SMS ini\">[Balas]</a>"."     "."<a href=pmiadmin.php?module=hapus_sms_inbox&Id=".$id." TITLE=\"Hapus SMS ini\">[Hapus]</a>";
				}else{
					/* General output */
					$row[] = $aRow[ $aColumns[$i] ];
				}
