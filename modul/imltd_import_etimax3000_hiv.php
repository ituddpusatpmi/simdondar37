...
if (!empty($_FILES["filecsv"]["tmp_name"])){
	...
	$jml=0;
	?>
	<a name="atas"><h2>HASIL IMPORT ANTI-HIV, File : <?=$_FILES['filecsv']['name']?></h2>
	...
	<?php	
		while (($data = fgetcsv($datacsv, 1000, $pemisah)) !== FALSE){
			...
			if (($jml==7) or ($jml==8) or ($jml==9)){
				if($data[2]=="FAILED"){$kontrolvalid=1;}
			}
			//CUT-OFF VALUE : NC+0.150
			if($data[0]=="A1"){$nc1=$data[4];} if($data[5]=="*"){$gagalimport=1;}
			if($data[0]=="B1"){$nc2=$data[4];} if($data[5]=="*"){$gagalimport=1;}
			if($data[0]=="C1"){$pc1=$data[4];} if($data[5]=="*"){$gagalimport=1;}
			if($data[0]=="D1"){$pc24=$data[4];} if($data[5]=="*"){$gagalimport=1;}
			$ncx=((float)$nc1+(float)$nc2)/2;
			...
			if (($gagalimport==0) and ($kontrolvalid==0)){
				if ($jml>11 and $data[0]<>"[End Of Results]" and strlen($data[1])>0 and $data[1]<>"Patient ID") {
					...
					if ($ktg==0){
						$sql="insert into imltd_import_temp (tanggal,nokantong,hiv_cut_off,hiv_quanti,hiv_reader,hiv_result,hiv_od,reagen_hiv,hiv_metode)
							    values('$tanggal','$data[1]','$cut_off','$data[3]','$data[4]','$data[5]','$od','$reag0_ex[0]','$reag0_ex[5]')";
						$tambah=mysql_query($sql,$con);
						$add++;
					}else{
						$sql=	"update imltd_import_temp set tanggal = '$tanggal', hiv_cut_off = '$cut_off', hiv_quanti = '$data[3]', hiv_reader = '$data[4]',
								hiv_result = '$data[5]', hiv_od = '$od', reagen_hiv = '$reag0_ex[0]', hiv_metode = '$reag0_ex[5]' where nokantong = '$data[1]'";
						$tambah=mysql_query($sql,$con);
						$upd++;
					}
					?>
	...
	<a href="pmiimltd.php?module=import_etimax3000syp"class="swn_button">Lanjut ke Syphilis</a>
...
<form action="" method="post" enctype="multipart/form-data" name="ContohUploadCSV" id="ContohUploadCSV">
	<h2>ANTI-HIV</h2>
	<h2>LANGKAH IMPOR ke 3 dari 4 langkah</h2>
	<table border=0>
	<tr>
		<td>Pilih Reagen Diasorin/Murex Anti-HIV
		<select name="reagen2" id="reagen2" onChange="show0(2)" STYLE="width: 300px">
			<option value="">Pilih Reagen Diasorin/Murex Anti-HIV</option>
				<? 
				$jreagen1=mysql_query("select * from reagen where ((Nama like '%Diasorin%hiv%') or (Nama like '%Murex%hiv%')) and aktif='1' and jumTest>0");
	...
	<h2>Pilih file txt/csv untuk Parameter HIV</h2> 
    <input type="file" name="filecsv" id="filecsv" class="swn_button"/>
    <input type="submit" name="button" id="button" value="Proses Import Anti-HIV" class="swn_button"/>
</form>
<a href="pmiimltd.php?module=import_etimax3000syp"class="swn_button">Lewatkan</a>
<a href="pmiimltd.php?module=import_etimax3000"class="swn_button">Kembali ke awal</a>
<? }?>
</body>
</html>
