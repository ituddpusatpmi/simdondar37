...
$sql = "SELECT `mulaiPutar`, `selesaiPutar`, `mulaiPisah`, `selesaiPisah` FROM dpengolahan_temp WHERE `petugas` = '$namaPetugas' ORDER BY `id` DESC";
...
// $mPutar = $sPutar = $mPisah = $sPisah = '00:00'; // default isi waktu adalah 00:00
...
if ($result->num_rows > 0) {
    ...
    // Menggunakan nilai default jika hasil query kosong
...
<body>
            ...
            <div class="col-sm-4">
                <div class="form-group">
                    ...
                    <select class='custom-select' id="alatPemutaran" name="alatPemutaran">
                        <?php
                        $aPutar = mysqli_query($dbi, "SELECT id, kode, nama_barang FROM logbook_h WHERE fungsi LIKE '%pemutaran%'");
                        while ($aP = mysqli_fetch_assoc($aPutar)) { ?>
                            <option value="<?php echo $aP['kode']; ?>">
            ...
            <div class="col-sm-4">
                <div class="form-group">
                    ...
                    <select class='custom-select' id="alatPemisahan" name="alatPemisahan">
                        <?php
                        $aPisah = mysqli_query($dbi, "SELECT id, kode, nama_barang FROM logbook_h WHERE fungsi LIKE '%pemisahan%'");
                        while ($aPs = mysqli_fetch_assoc($aPisah)) { ?>
                            <option value="<?php echo $aPs['kode']; ?>">
        ...
        function simpanDanLanjutkan() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    //     if (response.success) {
                    //         window.location.href = 'pengolahan.php';
                    //     } else {
                    ...
                    // },
                    // error: function() {
                    //     $('#errorModal .modal-body').html('Terjadi kesalahan saat menyimpan data.');
                    //     $('#errorModal').modal('show');
                    ...
                    try {
                        ...
                        if (jsonResponse.status === 'success') {
                            ...
                            $('#suksesModal').on('hidden.bs.modal', function () {
                                //window.location.href = 'rincian_tr_konvensional.php?noTrans=' + encodeURIComponent(jsonResponse.noTrans);
    ...
    <script>
        $(document).ready(function () {
            $('#jMPutar, #jSPutar, #jMPisah, #jSPisah').clockpicker({
                autoclose: true, // Menutup otomatis setelah waktu dipilih
                placement: 'bottom', // Posisi kotak waktu (atas, bawah, kiri, kanan)
                align: 'left', // Penjajaran kotak waktu
                donetext: 'Selesai', // Teks pada tombol setelah waktu dipilih
                twelvehour: false // Format 24 jam (true untuk format 12 jam)
            });
        });
    </script>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            // Event listener untuk update warna saat slider digeser
        ...
        function updateSliderColor(slider) {
            if (slider.value == "1") {
                slider.style.background = "#2196F3"; // Jika 1, warna biru
            } else {
                // slider.style.background = "#ccc"; // Jika 0, warna abu-abu
