...
$array_bulan = array(1=>'Jan','Feb','Mar', 'Apr', 'Mei', 'Jun','Jul','Ags','Sep','Okt', 'Nov','Des');
	 ...
	 */
	$aColumns = array( 'InsertIntoDB', 'DestinationNumber', 'TextDecoded' ,'ID');
	...
	/* DB table to use */
	$sTable = "outbox";
	include("../config/db_connect_sms.php");
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			else if ( $aColumns[$i] != ' ' )
			{
				if($i==0){
					...
					$id=$aRow[ $aColumns[$i]];
					$hapus='
					<button id="'.$id.'" type="submit" onClick="hapus('.$id.')" 
							value="Simpan" name="submit" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" role="button" aria-disabled="false">
					    <span class="ui-button-text">Batalkan</span>
					</button>';
					$row[]=$hapus;
				}else{
					/* General output */
