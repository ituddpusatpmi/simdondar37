...
if ($kodeassay=="U2"){
		...
		for ($z=0;$z<96;$z++){
			...
			if ((strlen($sampleid[$nomor])!==0) and ($sampleid[$nomor]!=="NC1") and ($sampleid[$nomor]!=="PC1")){
				...
				if (($sampleid[$nomor]!=="") and ($threshold[$nomor]!=="*")){
					...
					if ($ktg==0){
						$sql="insert into imltd_import_temp (tanggal,    nokantong,          hcv_cut_off, hcv_reader,        hcv_result,           hcv_od,        reagen_hcv,   hcv_metode)
						...
						$sql=	"update imltd_import_temp set tanggal = '$tanggal', hcv_cut_off = '$cut_off', hcv_reader= '$odresult[$nomor]',
								hcv_result	= '$hasiltmp', hcv_od = '$ratio[$nomor]', reagen_hcv = '$reag0_ex[0]',
								hcv_metode	= '$reag0_ex[5]' WHERE nokantong = '$sampleid[$nomor]'";
						$tambah=mysql_query($sql,$con);
						$upd++;
					}
				}
	...
	<a href="pmiimltd.php?module=import_davincihiv"class="swn_button_blue">Lanjut ke Anti-HIV</a>
...
<?
	...
	echo ("<SCRIPT LANGUAGE='JavaScript'>
		window.alert('File tidak bisa diimport. Bukan Parameter Anti-HCV. Pastikan file yang dipilih adalah parameter Anti-HCV Ultra!!!!')
...
<form action="" method="post" enctype="multipart/form-data" name="UploadCSV" id="UploadCSV">
	<font size=6 color="blue"><b>Parameter HCV ULTRA</b></font><br>
	<table class="list" border="1" cellpadding="2" cellspacing="1">
		<tr align="center" class="field">
			<td>Pilih Reagen Biomerieux HCV Ultra
			<select name="reagen1" id="reagen1" onChange="show0(1)" STYLE="width: 300px">
				<option value="">Pilih reagen Biomerieux HCV Ultra</option>
				<? 
				$jreagen1=mysql_query("select * from reagen where ((Nama like '%Biomerieux%HCV%') or (Nama like '%Biomerieux%hcv%')) and aktif='1' and jumTest>0");
			...
			<td><div id='kode1'></div></td>
			<td><div id='b1'></div></td>
	</tr>
	</table>
	<font size=4 color="black"><b>Pilih File CSV</b></font><br>
    <input type="file" name="filecsv" id="filecsv" class="swn_button_blue" />
    <input type="submit" name="button" id="button" value="Proses Import HCV" class="swn_button_blue"/>
</form>
<a href="pmiimltd.php?module=import_davincihiv"class="swn_button_blue">Lanjutkan ke HIV</a>
<a href="pmiimltd.php?module=import_davinci"class="swn_button_blue">Kembali ke awal</a>
<? }?>
</body>
</html>
