...
if (isset($_POST[submit2])) {
			$tambah1=mysql_query("UPDATE stokkantong set StatTempat='2',tglmutasi='$today'
				where (StatTempat='0' or StatTempat is NULL)"); 
			if ($tambah1) { echo ("Data Kantong Telah Pindahkan Semua Ke Gudang MU !! <meta HTTP-EQUIV=\"REFRESH\" CONTENT=\"1; URL=$PHP_SELF\">"); }}

if (isset($_POST[submit1])) {
	$nkt=strtoupper($_POST[nokantong]);
	 $nkt1=mysql_real_escape_string($_POST[nokantong]);
   $nktc=substr($nkt1,0,strlen($nkt1)-1);
    $cek=mysql_query("SELECT noKantong FROM stokkantong WHERE noKantong like '$nkt'");
	if (mysql_num_rows($cek)==1){
		$cekTempat=mysql_fetch_assoc(mysql_query("SELECT StatTempat FROM stokkantong WHERE noKantong like '$nkt'"));
		if ($cekTempat[StatTempat]!='1'){
			$tambah=mysql_query("UPDATE stokkantong set StatTempat='2',tglmutasi='$today'
				where noKantong like '$nktc%'"); 
...
<input type=submit name=submit value=Submit>
	...
	<form name="tambahkantong" method="POST" action="<?=$PHPSELF?>">
		<table class="form">
			...
			<input name="submit1" type="submit" value="Simpan">  <input name="submit2" type="submit" value="Pindahkan Semua Ke Gudang MU">
	</form>

<? 
$data=mysql_query("select * from stokkantong where (StatTempat='0' or statTempat is null) and noKantong like '%A'");
if (isset($_POST[terima1])) $data=mysql_query("select * from stokkantong where CAST(tglTerima as date)>='$today' and CAST(tglTerima as date)<='$today1'  and noKantong like '%A' and StatTempat is null "); ?>
...
<table class="list" cellpadding=5>
	...
	<tr class="record">
		...
		<td> <a href="pmilogistik.php?module=pindahan_kantongmu&noKantong=<?=$data1[noKantong]?>"><?=$data1[noKantong]?></a></td>
		<td><?=$jenis?></td>
		<td><?=$tempat?></td>
	</tr>
<? } ?>
