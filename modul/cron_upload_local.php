...
$prod=mysql_query("select * from produk order by no ASC",$con);
...
$squmur=mysql_fetch_assoc(mysql_query("SELECT  IF((FLOOR((TO_DAYS(NOW())- TO_DAYS(TglLhr)) / 365.25)) <30,1,0)) as 'umur17_30',IF((FLOOR((TO_DAYS(NOW())- TO_DAYS(TglLhr)) / 365.25)) BETWEEN 31 and 40,1,0)) as 'umur31_40',
IF((FLOOR((TO_DAYS(NOW())- TO_DAYS(TglLhr)) / 365.25)) BETWEEN 41 and 50,1,0)) as 'umur41_50',IF((FLOOR((TO_DAYS(NOW())- TO_DAYS(TglLhr)) / 365.25)) BETWEEN 51 and 60,1,0)) as 'umur51_60',
    IF((FLOOR((TO_DAYS(NOW())- TO_DAYS(TglLhr)) / 365.25)) >60,1,0)) as 'umur60_lebih'FROM pendonor",$con));
...
$con_pmipusat=mysql_connect($server_ip,$server_usr,$server_pwd,$server_port);	
