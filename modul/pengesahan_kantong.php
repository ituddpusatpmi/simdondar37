...
if (isset($_POST[submit2])) {
			$tambah1=mysql_query("UPDATE stokkantong set StatTempat='1',tglmutasi='$today'
				where (StatTempat='0' or StatTempat is NULL)");
			if ($tambah1) { echo ("Data Kantong Telah Pindahkan Semua Keaftap !! <meta HTTP-EQUIV=\"REFRESH\" CONTENT=\"1; URL=$PHP_SELF\">"); }}
...
<input type=submit name=submit value=Submit>
	...
	<form name="tambahkantong" method="POST" action="<?=$PHPSELF?>">
		<table class="form">
			...
			<input name="submit1" type="submit" value="Simpan">  <input name="submit2" type="submit" value="Pindahkan Semua KeAftap">
	</form>

<? 
$data=mysql_query("select * from stokkantong where (StatTempat='0' or statTempat is null) and noKantong like '%A'");
if (isset($_POST[terima1])) $data=mysql_query("select * from stokkantong where CAST(tglTerima as date)>='$today' and CAST(tglTerima as date)<='$today1' and noKantong like '%A' and (StatTempat='0' or StatTempat is null)"); ?>
...
//            case "FT":  $metkantong ="FILTER";       break;
	...
	<tr class="record">
		...
		<td> <a href="pmilogistik.php?module=pindahan_kantong&noKantong=<?=$data1[noKantong]?>"><?=$data1[noKantong]?></a></td>
		<td><?=$jenis?></td>
		<td><?=$tempat?></td>
	</tr>
<? } ?>
