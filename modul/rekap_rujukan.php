...
<h1 class="table">Rekap Pengiriman Rujukan Sample Tangal :   <?=$pertgl?> - <?=$perbln?> - <?=$perthn?> sampai <?=$pertgl1?> - <?=$perbln1?> - <?=$perthn1?>
</h1>
<div>
...
$no=1;
//(notrans,nosample,nokantongasal,jenis,tglinput,volume,gol_darah,RhesusDrh,uddtujuan,kodependonor,kodependonor_lama,tgl_Aftap,kadaluwarsa,tglperiksa,tglkirim,petugas)
$q_dtransaksipermintaan=mysql_query("select notrans,nosample,nokantongasal,jenis,status,tglinput,volume,gol_darah,RhesusDrh,uddtujuan,kodependonor,kodependonor_lama,tgl_Aftap,kadaluwarsa,tglperiksa,tglkirim,petugas from rujukanbaru where tglinput>='$today' and tglinput<='$today1' and uddtujuan is not NULL order by tglinput");
$TRec=mysql_num_rows($q_dtransaksipermintaan);
while($a_dtransaksipermintaan=mysql_fetch_assoc($q_dtransaksipermintaan)){
	$q_stok=mysql_query("select gol_darah,produk,RhesusDrh from stokkantong where noKantong='$a_dtransaksipermintaan[nokantongasal]' ");
//	$q_dhasilcross=mysql_query("select Pemeriksa from dhasilcross where noKantong='$a_dtransaksipermintaan[NoKantong]' ");
	...
	$utd=mysql_fetch_assoc(mysql_query("select nama from utd where id='$a_dtransaksipermintaan[uddtujuan]'"));
...
<?
$blnkel=substr($a_dtransaksipermintaan[tglinput],5,2);
$tglkel=substr($a_dtransaksipermintaan[tglinput],8,2);
$thnkel=substr($a_dtransaksipermintaan[tglinput],0,4);
?>
      	<td class=input><?=$tglkel?>-<?=$blnkel?>-<?=$thnkel?></td>
	<td class=input><?=$a_dtransaksipermintaan[nosample]?></td>
	<td class=input><?=$a_dtransaksipermintaan[nokantongasal]?></td>
<!--?
$jenis='Double';
...
?-->
	<td class=input><?=$a_dtransaksipermintaan[jenis]?></td>
	<td class=input><?=$a_dtransaksipermintaan[status]?></td>
...
<br>
<form name=xls method=post action=modul/rekap_terima_darah_udd_lain_xls.php>
...
<input type=submit name=submit2 value='Print Rekap terima darah (.XLS)'>
</form>
