...
$jd=mysql_query("select * from pendonor where 
		 ...
		 COALESCE (Alamat,'') like '%$srcalamat%' and COALESCE (kelurahan,'') like '%$srckelurahan%' and COALESCE (kecamatan,'') like '%$srckecamatan%' and 			 COALESCE (wilayah,'') like '%$srcwilayah%' and COALESCE (TempatLhr,'') like '%$srctmptlhr%' and COALESCE (TglLhr,'') like '%$srctgllahir%' and 		 COALESCE (Rhesus,'') like '%$srcrhesus%' and COALESCE (GolDarah,'') = '$src_abo' and COALESCE (telp2,'') like '%$srctelp%' and
		 COALESCE (apheresis,'') = '1' ORDER BY respon_aph, tglkembali_apheresis ASC");
