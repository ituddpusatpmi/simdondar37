...
if (isset($_POST[submit])) {
	...
	$sekarang = date("Y-m-d h:m:s");
	 ...
	 $tambah_sql="insert into pendonor 
					...
					`pencatat`,`mu`,`cekal`,`up`,`waktu_update`,`tanggal_entry`,`apheresis`,`waktu_apheresis`)
			   values ('$kode','$noktp','$nama','$alamat','$jk','$pekerjaan',
					'$telp','$tempatlhr','$tgllhr','$status','$goldarah',
					'$rhesus','$call','$kelurahan','$kecamatan','$wilayah','$kodepos','$jumdonor','-',
					'$telp2','$umur','$sekarang','$ibukandung',
					'$namauser','$mu','0','1','$sekarang','$sekarang','$apheresis','$wktaph')";
	...
	//=======Audit Trial==0==========================================================================
	$log_mdl ='REGISTRASI';
	$log_aksi='Manambah data: '.$kode.' - '.$nama;
	include_once "user_log.php";

$selins=mysql_fetch_assoc(mysql_query("SELECT CONCAT_WS(' | ','No.KTP','$noktp |') as col1,
            CONCAT_WS(' | ','Nama','$nama |') as col2,
            CONCAT_WS(' | ','Jenis Kelamin','$jeniskelamin|') as col3,
            CONCAT_WS(' | ','Tempat Lahir','$tempatlhr |') as col4,
            CONCAT_WS(' | ','Tgl Lahir','$tgllhr |') as col5,
            CONCAT_WS(' | ','Pekerjaan','$pekerjaan |') as col6,
            CONCAT_WS(' | ','Umur','$umur |') as col7,
            CONCAT_WS(' | ','Status Menikah','$menikah|') as col8,
            CONCAT_WS(' | ','No.HP','$telp2 |') as col9,
            CONCAT_WS(' | ','Telp','$telp |') as col10,
            CONCAT_WS(' | ','Dipanggil','$dipanggil|') as col11,
            CONCAT_WS(' | ','ABO (Rh)','$goldarah ($rhesus) |') as col12,
            CONCAT_WS(' | ','Alamat','$alamat |') as col13,
            CONCAT_WS(' | ','Kelurahan','$kelurahan |') as col14,
            CONCAT_WS(' | ','Kecamatan','$kecamatan |') as col15,
            CONCAT_WS(' | ','Wilayah','$wilayah |') as col16,
            CONCAT_WS(' | ','Kode Pos','$kodepos |') as col17,
            CONCAT_WS(' | ','Jumlah Donor','$_POST[jumdonor] |') as col18,
            CONCAT_WS(' | ','Ibu Kandung','$ibukandung') as col19
            FROM pendonor WHERE Kode='$kode'"));

    $inshist=mysql_query("INSERT INTO histori
             (notrans, username, level_editor, waktu, KodePendonor, action, jenis, tempat, up)
             VALUES
             ('$id_transaksi_baru','$namauser','$_SESSION[leveluser]', '$sekarang', '$kode',
             '$selins[col1] $selins[col2] $selins[col3] $selins[col4] $selins[col5] $selins[col6] $selins[col7] $selins[col8] $selins[col9] $selins[col10] $selins[col11] $selins[col12] $selins[col13] $selins[col14] $selins[col15] $selins[col16] $selins[col17] $selins[col18] $selins[col19]', '0', '$temp', '$up')");
	...
	if ($tambah) {
		  ...
		  switch ($lv0){
			  ...
			  case "p2d2s":
					?>
				<META http-equiv="refresh" content="0; url=pmip2d2s.php?module=transaksi_donor&Kode=<?=$kode?>">
					<?
			   break;
			   case "kasir":
			   ...
			   break;
			   case "laboratorium":
					?><META http-equiv="refresh" content="0; url=pmilaboratorium.php?module=transaksi_donor&Kode=<?=$kode?>"><?
			   break;
                           case "bdrs":
...
switch ($lv0) {
	 ...
	 break;
	case "p2d2s":
		  ?><form name="reg" autocomplete="off" method="post" action="pmip2d2s.php?module=registrasi"><?
	 break;
	 case "kasir":
	...
	case "aftap":
	 ...
	 break;
	case "laboratorium":
		  ?><form name="reg" method="post" action="pmilaboratorium.php?module=registrasi"><?
	 break;
         case "bdrs":
	 ...
	 <tr>
		  <td>
			   ...
			   <tr>
					<td>No. KTP/SIM</td>
					<td class="input">
						 <input name="noktp" type="text" size="30" required placeholder="Nomor KTP/SIM/KP/KTM" >
					</td>
			   </tr>
			   <tr>
					<td>Nama</td>
					<td class="input">
						 <input name="nama" type="text" size="30" required placeholder="Nama lengkap, GELAR dibelakang" pattern="^[A-Za-z.,' ]{3,}$">
					</td>
			   </tr>
			   <tr>
					<td>Alamat</td>
		  ...
		  <td>
			   ...
			   <tr>
			    <td >Jumlah Donor</td>
					<td class="input">
						 <input name="jumdonor" required type="text" size="4" >
					</td>
			   </tr>
			   <tr>
			   ...
			   <tr>
			     <td>Waktu Apheresis</td>
			      <td class="input">
				<select name="wktaph">
				  <option value="0">Kapan saja</option>
