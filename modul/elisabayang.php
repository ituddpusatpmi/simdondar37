...
<script language=javascript src="js/elisabayang.js" type="text/javascript"> </script>
    ...
    for($j=0;$j<4;$j++){
		...
		if ($rg!='') {
			...
			for($i=0;$i<$_POST[jum_kantong];$i++){
				...
				if (($metode=='rapid') and ($metode=='elisa') and ($metode=='clia')) $hs=$od;
				$od=str_replace(',','.',$od);
				...
				$seldonor=mysql_fetch_assoc(mysql_query("SELECT JenisDonor, umur, donorbaru FROM htransaksi WHERE nokantong='$nkt'"));
				$ratio=$od;
				if ($co>0) $ratio=$od/$co;
					if ($od!='') {
						if ($metode=='elisa') {
							$tambah_sql="insert into hasilelisa (
										...
										dicatatOleh,dicekOleh,DisahkanOleh,nolot,Metode,ulang, umur, jns_donor, baru_ulang)
									values ('$nkt','0','0','0','$notest','$njt1','$today',
										'$namauser','$_POST[sah1]','$_POST[sah]','$nolot','elisa','0','$seldonor[umur]','$seldonor[JenisDonor]','$seldonor[donorbaru]')";
							...
							$tambah_sql="insert into hasilelisa (
										...
										dicatatOleh,dicekOleh,DisahkanOleh,nolot,Metode,ulang, umur, jns_donor, baru_ulang)
									values ('$nkt','0','0','0','$notest','$njt1','$today',
										'$namauser','$_POST[sah1]','$_POST[sah]','$nolot','nat','0','$seldonor[umur]','$seldonor[JenisDonor]','$seldonor[donorbaru]')";
							$tambah=mysql_query($tambah_sql);
							//echo "<br> $tambah_sql <br>";
							...
							$tambah=mysql_query("insert into drapidtest (NoTrans,noKantong,Kontrol,jenisPeriksa,Hasil,nolot,tgl_tes,dicatatoleh,dicekOleh,DisahkanOleh,Metode,ulang) 
											values ('$notest','$nkt','$kontrol','$njt1','1','$nolot','$today','$namauser','$_POST[sah1]','$_POST[sah]','Rapid','0')");
						}
					}
			    	}
		    ...
		    if (($nrapid1+$nelisa1+$nclia1)=='4') {
				...
				$ncliagz=mysql_num_rows(mysql_query("select * from hasilclia where noKantong='$nkt' and Hasil='2'"));
				...
				if ($nrapid==0 and $nelisa==0 and $nclia==0) {
						
						
					if ($nelisagz==0 and $ncliagz==0) {
								$nsehat++;
				...
				//	}
					// end ucapan
						$tambah3=mysql_query("UPDATE pendonor SET Cekal='0' where Kode='$idpendonor'");	//bebas cekal
						if ($jum_komponen>0) { $tambah3s=mysql_query("UPDATE stokkantong set Status='2',tglpengolahan=tgl_Aftap, tglperiksa='$today', sah='1',hasil='2',StatTempat='1'
 where NoKantong like '$no_kantong0%'"); }
							else { $tambah3s=mysql_query("UPDATE stokkantong set Status='2',tglpengolahan=tgl_Aftap,tglperiksa='$today', sah='1', hasil='2',StatTempat='1'  where NoKantong='$nkt'"); }
/*
						switch ($jum_komponen){
							case 1:
...
*/
					...
					$nrusak++;
						if ($jum_komponen>0) { $tambah3s=mysql_query("UPDATE stokkantong set Status='7',tglpengolahan=tgl_Aftap, tglperiksa='$today', sah='1',hasil='4',StatTempat='1'  where NoKantong like '$no_kantong0%'");}
							else { $tambah3s=mysql_query("UPDATE stokkantong set Status='7',tglpengolahan=tgl_Aftap,tglperiksa='$today',sah='1',StatTempat='1', hasil='4' where NoKantong='$nkt'"); }
/*
						switch ($jum_komponen){
							case 1:
...
*/
	...
	//---End Cek sudah di tes berapa kali?----
		...
		<li>$nrusak &nbsp;-> &nbsp;Darah Rusak</li><li>$ngz &nbsp;->&nbsp;Grey Zone no kantong $no_kantong0, jumlah komponen $jum_komponen</li></ol>";
				...
				<select name="reagen0" id="reagen0" onChange="show0(0)" STYLE="width: 190px">
					...
					while ($jreagen11=mysql_fetch_assoc($jreagen1)) { 
						...
						if ($merk11['nama_reagen']!='') {
							...
							<?=$jreagen11[Nama]?>-<?=$jreagen11[noLot]?>_<?=$jreagen11[jumTest]?> test</option><?
						}
					} ?>
				</select>
			</td>
...
<option value="<?=$jreagen21[kode]?>*<?=$jreagen21[jumTest]?>*<?=$m_reagen2[reaktif]?>*<?=$m_reagen2[nonreaktif]?>*<?=$m_reagen2[greyzone]?>*<?=$jreagen21[metode]?>">
							<?=$jreagen21[Nama]?>-<?=$jreagen21[noLot]?>_<?=$jreagen21[jumTest]?> test</option><?
						}
					}?>
				</select>
			</td>
...
<option value="<?=$jreagen31[kode]?>*<?=$jreagen31[jumTest]?>*<?=$m_reagen3[reaktif]?>*<?=$m_reagen3[nonreaktif]?>*<?=$m_reagen3[greyzone]?>*<?=$jreagen31[metode]?>"><?=$jreagen31[Nama]?>-<?=$jreagen31[noLot]?>_<?=$jreagen31[jumTest]?> test</option><?
						}
					}?>
				</select>
			</td>
...
<option value="<?=$jreagen41[kode]?>*<?=$jreagen41[jumTest]?>*<?=$m_reagen4[reaktif]?>*<?=$m_reagen4[nonreaktif]?>*<?=$m_reagen4[greyzone]?>*<?=$jreagen41[metode]?>"><?=$jreagen41[Nama]?>-<?=$jreagen41[noLot]?>_<?=$jreagen41[jumTest]?> test</option><?
						}
					}?>
				</select>
			</td>
