...
?>
<body>
<font size="4" color="red" font-family="Arial">REKAP PEMERIKSAAN UJI SARING DARAH (IMLTD)</font><br>
...
if (isset($_POST[submit])){
	...
	$sql="select distinct hasil_imltd.`nomor`, hasil_imltd.`tanggal`, hasil_imltd.`pemeriksa`, round(count(hasil_imltd.`nomor`)/4) as jumlah
			from hasil_imltd
			where (date(hasil_imltd.tanggal)>='$_POST[waktu]' and date(hasil_imltd.tanggal)<='$_POST[waktu1]')
			group by
			hasil_imltd.`nomor`, hasil_imltd.`tanggal`, hasil_imltd.`pemeriksa`
			";
...
<table class="list" cellpadding=5 cellspacing=1>
	...
	<tr class="record">
		...
		<td align="left"><?=$data1[nomor]?></a></td>
		<td align="left"><?=$data1['tanggal']?></td>
		<td align="right"><?=$data1['jumlah']?></td>
		<td align="left"><?=$data1['pemeriksa']?></td>
		<td>
			<a href=pmilaboratorium.php?module=tampilhasil_imltd&notrans=<?=$data1[nomor]?>&tanggal=<?=$data1[tanggal]?>>Tampilkan hasil</a>
		</td>
	</tr>
	<? } ?>
</table>
