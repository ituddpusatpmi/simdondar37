...
$umur=mysql_fetch_assoc(mysql_query("select umur as um ,JenisDonor as jd ,jk as jk,donorbaru as db from htransaksi  where nokantong='$baris[nk]'"));
        ...
        $reak3=mysql_query("select Hasil,tglPeriksa,dicatatOleh,OD,Metode from hasilclia where noKantong='$baris[nk]' and jenisPeriksa='$jenis' limit 1");
         if (mysql_num_rows($reak3)=='1') {
        $reak4=mysql_fetch_assoc($reak3);
        $hasilr='Non Reaktif';
        if ($reak4[Hasil]=='1') $hasilr='Reaktif';

        ?>
                <td><?=$hasilr?> , <?=$reak4[OD]?></td>
                <?
        $tgl=$reak4[tglPeriksa];

        $pemeriksa=$reak4[dicatatOleh];
        $metode=$reak4[Metode];
        $umur=mysql_fetch_assoc(mysql_query("select umur as um ,JenisDonor as jd ,jk as jk,donorbaru as db from htransaksi  where nokantong='$baris[nk]'"));
             
         } else {
             $reak5=mysql_query("select Hasil,tglPeriksa,OD from hasilnat where noKantong='$baris[nk]' and jenisPeriksa='$jenis' limit 1");
             $reak6=mysql_fetch_assoc($reak5);
             $hasilr='Non Reaktif';
             if ($reak6[Hasil]=='1') $hasilr='Reaktif';

             ?>
                     <td><?=$hasilr?> , <?=$reak6[OD]?></td>
                     <?
		$detail=mysql_fetch_assoc(mysql_query("select dicatatOleh,Metode from hasilnat where noKantong='$baris[nk]' limit 1"));
             $tgl=$reak6[tglPeriksa];

             $pemeriksa=$detail[dicatatOleh];
