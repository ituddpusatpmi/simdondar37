...
<script language=javascript src="../js/imltd_import_etimax3000.js" type="text/javascript"> </script>
<style type="text/css">
...
$jmlperiksa=1;
$msg="Hasil proses import hasil IMLTD Diasorin Eti-Max 3000. ";
...
if(isset($_POST['Button']))  {
		...
		if ($status_kantong==1){
			...
			if (($nrapid1+$nelisa1)>='4') {
				...
				if ($nrapid==0 and $nelisa==0){
					...
					$nrusak++;
					if ($jum_komponen>0) {
						$upd_kantong_rusak=mysql_query("UPDATE stokkantong set Status='7',hasil='4',tglpengolahan=tgl_Aftap, tglperiksa='$today',ident='m'
	...
	?><META http-equiv="refresh" content="0; url=pmiimltd.php?module=import_etimax3000"><?
} else if (isset($_POST['manualhbsag']))  {
	$jmlsample=$_POST['jumlahtotal'];
	$URL="pmiimltd.php?module=import_etimax3000manual&parameter=hbsag&jmlperiksa=+$jmlsample";
	header("Location: $URL");
} else if (isset($_POST['manualhcv']))  {
	$jmlsample=$_POST['jumlahtotal'];
	$URL="pmiimltd.php?module=import_etimax3000manual&parameter=hcv&jmlperiksa=+$jmlsample";
	header("Location: $URL");
}else if (isset($_POST['manualhiv']))  {
	$jmlsample=$_POST['jumlahtotal'];
	$URL="pmiimltd.php?module=import_etimax3000manual&parameter=hiv&jmlperiksa=$jmlsample";
	header("Location: $URL");
}else if (isset($_POST['manualsyp']))  {
	$jmlsample=$_POST['jumlahtotal'];
	$URL="pmiimltd.php?module=import_etimax3000manual&parameter=syp&jmlperiksa=$jmlsample";
	header("Location: $URL");
}
...
?>
	...
	?>
	<font size=4 color="red"><b>KONFIRMASI PROSES HASIL IMPORT DIASORIN ETI-MAX 3000</b></font><br>
	<a name="atas"></a>
	...
	while($baris=mysql_fetch_assoc($hasil)){
			...
			<tr class="record">
				...
				//edit non reaktif => Neg
				if ($baris[hbsag_result]=="Neg"){
					?><td><?=$baris[hbsag_result]?></td><input type="hidden" name=hbsag_result[] value="0"><?
				} else if ($baris[hbsag_result]=="Pos") {
	...
	<tr class="field">
		...
		<a href="pmiimltd.php?module=import_etimax3000"class="swn_button" title="Kembali ke menu Diasorin">Kembali ke awal</a>
		<?if ($param_all==1){?>
