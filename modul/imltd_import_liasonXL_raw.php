...
<body>
    <font size="5" color=00008B>DATA TRANSFER PEMERIKSAAN DIASORIN LIAISON<sup>&reg</sup>XL</b></font><br><br>
    ...
    <?php
    $no=0;
    $sql="SELECT `id_transaksi`,`instrument_name`,`run_time`, date(`run_time`) as tanggal, `operator`, count(`sample_id`) as jml_sample
        FROM `liason2021`
        WHERE `confirm`='0' and `sample_type`='S' and (qualitative in ('-1','0','1'))
        GROUP BY `instrument_name`, `id_transaksi`,`operator`";
    $qraw=mysql_query($sql,$con);
    while($tmp=mysql_fetch_assoc($qraw)){
        ...
        <tr style="font-size:13px; color:#000000; font-family:Verdana;" onMouseOver="this.className='highlight'" onMouseOut="this.className='normal'">
            ...
            <td>
                <a href="pmiimltd.php?module=import_liasonxl_konfirm&ins=<?=$tmp['id_transaksi']?>&tgl=<?=$tmp['tanggal']?>&usr=<?=$tmp['operator']?> "class="swn_btn_kiri">Konfirm</a> |
                <a href="pmiimltd.php?module=import_liasonxl_del&no=<?=$tmp['run_time']?>" onclick="return confirm('PERHATIAN \n \nYakin akan menghapus data transfer nomor : <?=$tmp['run_time']?> ???');" class="swn_btn_tengah">Hapus</a>
