...
if ($_POST[submit]) {
   ...
   $volmin 	= $_POST['volmin'];
   $volmax 	= $_POST['volmax'];
   $volmin450 	= $_POST['volmin450'];
   $volmax450 	= $_POST['volmax450'];
   $nama  	= $_POST['nama'];
   ...
   for ($i=0;$i<count($nomor);$i++) {
    $simpan=mysql_query("update produk set
                        ...
                        vol_min     ='$volmin[$i]',
                        vol_max     ='$volmax[$i]',
		 	vol_min450  ='$volmin450[$i]',
                        vol_max450  ='$volmax450[$i]',
...
$sq=mysql_query("SELECT `Nama`, `no`, `kantongbaru`, `lengkap`, `umurhari`, `umurjam`, `suhusimpan`,
                `volume`, `beratjenis`, `vol_min`, `vol_max`, `vol_min450`, `vol_max450`, 
	        `waktu_pengolahan`, `lama_pengambilan`  FROM `produk` order by `no`");
?>
...
<form name='transaksi'  method='post' action='<?=$PHPSELF?>'>
    <table class="list" border="1" cellspacing="4" cellpadding="5" style="border-collapse:collapse">
      <tr class="field">
		...
		<td align="center" colspan="2">VOLUME RELEASE 350 (ml)</td>
		<td align="center" colspan="2">VOLUME RELEASE 450 (ml)</td>
	  ...
	  <tr class='record'>
			...
			<td class='input'><input name='volmin[]' type='text' size='10'       value=<?=$dtrans['vol_min']?> style='text-align:right'></td>
			<td class='input'><input name='volmax[]' type='text' size='10'       value=<?=$dtrans['vol_max']?> style='text-align:right'></td>
			<td class='input'><input name='volmin450[]' type='text' size='10'       value=<?=$dtrans['vol_min450']?> style='text-align:right'></td>
			<td class='input'><input name='volmax450[]' type='text' size='10'       value=<?=$dtrans['vol_max450']?> style='text-align:right'></td>
