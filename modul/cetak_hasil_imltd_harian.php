...
<?php
  ...
  $sql="select distinct  hasil_imltd.nokantong as nokantong, hasil_imltd.nomor, hasil_imltd.pemeriksa as pemeriksa ,htransaksi.instansi as instansi from hasil_imltd inner join htransaksi on htransaksi.nokantong=hasil_imltd.nokantong
		where date(hasil_imltd.tanggal)='$tanggal' order by hasil_imltd.nomor";
...
<font size="3" color="black" font-family="Arial">HASIL PEMERIKSAAN UJI SARING DARAH DONOR (IMLTD) HARIAN</font><br>
<font size="3" color="black" font-family="Arial">Tanggal : <?=$tanggal?></font><br>
<br>
<table class="list" border="1" cellspacing="2" cellpadding="2" style="border-collapse:collapse">
	<tr class="field">
		...
		<td colspan=3 rowspan=2>Pemeriksa</td>
	</tr>
	<tr class="field">
	...
	while ($data1=mysql_fetch_assoc($data)) { 
		...
		$sq_b="select * from hasilelisa where noKantong='$data1[nokantong]' and jenisPeriksa='0'";
		$dt_b=mysql_fetch_assoc(mysql_query($sq_b));
		if ($dt_b['Hasil']!==''){
			$od_b=$dt_b['OD'];
			$hasil_b=$dt_b['Hasil'];
			$cov_b=$dt_b['COV'];
			if ($dt_b['Hasil']=='0'){$hasil_b="NR";}if ($dt_b['Hasil']=='1'){$hasil_b="R";}
			$metode_b="Elisa";$dicatat=$dt_b['dicatatOleh']; $dicek=$dt_b['dicekOleh'];$disahkan=$dt_b['DisahkanOleh'];
		} else{
			$sq_b="select * from drapidtest where noKantong='$data1[nokantong]' and jenisperiksa='0'";
			$dt_b=mysql_fetch_assoc(mysql_query($sq_b));
			$hasil_b=$dt_b['Hasil'];
			if ($dt_b['Hasil']=='1'){$hasil_b="NR";}if ($dt_b['Hasil']=='0'){$hasil_b="R";}
			$metode_b="Rapid";$dicatat=$dt_b['dicatatoleh'];$dicek=$dt_b['dicekOleh'];$disahkan=$dt_b['DisahkanOleh'];
		}
		$sq_c="select * from hasilelisa where noKantong='$data1[nokantong]' and jenisPeriksa='1'";
		$dt_c=mysql_fetch_assoc(mysql_query($sq_c));
		if ($dt_c['Hasil']!==''){
			$od_c=$dt_c['OD'];
			$hasil_c=$dt_c['Hasil'];
			$cov_c=$dt_c['COV'];
			if ($dt_c['Hasil']=='0'){$hasil_c="NR";}if ($dt_c['Hasil']=='1'){$hasil_c="R";}
			$metode_c="Elisa";$dicatat=$dt_c['dicatatOleh']; $dicek=$dt_c['dicekOleh'];$disahkan=$dt_c['DisahkanOleh'];
		} else{
			$sq_c="select * from drapidtest where noKantong='$data1[nokantong]' and jenisperiksa='1'";
			$dt_c=mysql_fetch_assoc(mysql_query($sq_c));
			$hasil_c=$dt_c['Hasil'];
			if ($dt_c['Hasil']=='1'){$hasil_c="NR";}if ($dt_c['Hasil']=='0'){$hasil_c="R";}
			$metode_c="Rapid";$dicatat=$dt_c['dicatatoleh'];$dicek=$dt_c['dicekOleh'];$disahkan=$dt_c['DisahkanOleh'];
		}
		$sq_i="select * from hasilelisa where noKantong='$data1[nokantong]' and jenisPeriksa='2'";
		$dt_i=mysql_fetch_assoc(mysql_query($sq_i));
		if ($dt_i['Hasil']!==''){
			$od_i=$dt_i['OD'];
			$cov_i=$dt_i['COV'];
			$hasil_i=$dt_i['Hasil'];
			if ($dt_i['Hasil']=='0'){$hasil_i="NR";}if ($dt_i['Hasil']=='1'){$hasil_i="R";}
			$metode_i="Elisa";$dicatat=$dt_i['dicatatOleh']; $dicek=$dt_i['dicekOleh'];$disahkan=$dt_i['DisahkanOleh'];
		} else{
			$sq_i="select * from drapidtest where noKantong='$data1[nokantong]' and jenisperiksa='2'";
			$dt_i=mysql_fetch_assoc(mysql_query($sq_i));
			$hasil_i=$dt_i['Hasil'];
			if ($dt_i['Hasil']=='1'){$hasil_i="NR";}if ($dt_i['Hasil']=='0'){$hasil_i="R";}
			$metode_i="Rapid";$dicatat=$dt_i['dicatatoleh'];$dicek=$dt_i['dicekOleh'];$disahkan=$dt_i['DisahkanOleh'];
		}
		$sq_v="select * from hasilelisa where noKantong='$data1[nokantong]' and jenisPeriksa='3'";
		$dt_v=mysql_fetch_assoc(mysql_query($sq_v));
		if (($dt_v['Hasil']=='0') or ($dt_v['Hasil']=='1')){
			$od_v=$dt_v['OD'];
			$cov_v=$dt_v['COV'];
			$hasil_v=$dt_v['Hasil'];
			if ($dt_v['Hasil']=='0'){$hasil_v="NR";}if ($dt_v['Hasil']=='1'){$hasil_v="R";}
			$metode_v="Elisa";$dicatat=$dt_v['dicatatOleh']; $dicek=$dt_v['dicekOleh'];$disahkan=$dt_v['DisahkanOleh'];
		} else{
			$sq_v="select * from drapidtest where noKantong='$data1[nokantong]' and jenisperiksa='3'";
			$dt_v=mysql_fetch_assoc(mysql_query($sq_v));
			$hasil_v=$dt_v['Hasil'];
			if ($dt_v['Hasil']=='1'){$hasil_v="NR";}if ($dt_v['Hasil']=='0'){$hasil_v="R";}
	...
	<tr class=record>
		<td colspan=10 align="left">
			Ket : NR= Non Reaktif, R=Reaktif, I=Intermediate GZ:Greyzone
		</td>
