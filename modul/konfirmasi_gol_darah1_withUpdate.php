...
if (isset($_POST[terima])) {
	for ($i=0; $i<sizeof($_POST[goldarah]); $i++) {
		...
		//$nkt=substr($_POST[nokantong],0,-1);
        ...
        $q_cek_gol=mysql_query("SELECT gol_darah,RhesusDrh,produk,kodependonor from stokkantong where noKantong='$nkt'");
	...
	$tambah3=mysql_query("UPDATE htransaksi set gol_darah='$goldarah',rhesus='$rhesus' where KodePendonor='$a_cek_gol[kodependonor]'");
	$hkomp=mysql_query("select * from dkonfirmasi where NoKantong ='$nkt' ");
	if (mysql_num_rows($hkomp)=='1') {
	$update=mysql_query("UPDATE dkonfirmasi set GolDarah='$goldarah', Rhesus='$rhesus', ket='$_POST[ket]', tgl='$today3', petugas='$namauser',
	Cocok='$Cocok', goldarah_asal='$a_cek_gol[gol_darah]' ,rhesus_asal='$a_cek_gol[RhesusDrh]', metode='$metode', sel='0', antiA='$antiA', antiB='$antiB',
	antiO='-', serum='0', tA='$tA', tB='$tB', tsO='$ser', antiD='$antiD', ac='$ac', ba='$ba', nolot_aa='$_POST[nolota]', expa='$_POST[expa]',
	nolot_ab='$_POST[nolotb]', expb='$_POST[expb]', nolot_ad='$_POST[nolotd]', expd='$_POST[expd]' where NoKantong ='$nkt'");
}else{     
	...
	if ($tambah) {
        echo "Golongan Darah dan Rhesus Telah berhasil diupdate";?>
	<?}
} ?>
  ...
  onMouseOver="this.className='highlight'" onMouseOut="this.className='normal'" >
					...
					<tr >
						...
						<!--td class="input">NOLOT BA<INPUT type="text"  name="nolotba" size='9' required placeholder="nolot/kod.prod">
						</td>
						<td class="input">EXP. BA<INPUT type="text"  name="expba" size='9' required placeholder="yyyy-mm-dd">
						</td>

						<td class="input">NOLOT AK<INPUT type="text"  name="nolotak" size='9' required placeholder="nolot/kod.prod">
						</td>
						<td class="input">EXP. AK<INPUT type="text"  name="expak" size='9' required placeholder="yyyy-mm-dd"-->
						</td>
...
<table>
			...
			<table class="form" id="box-table-b" width=350px align=top>
					<tr class="field">
