...
$array_bulan = array(1=>'Jan','Feb','Mar', 'Apr', 'Mei', 'Jun','Jul','Ags','Sep','Okt', 'Nov','Des');
	...
	$aColumns = array( 'kode','nama_barang','tipe', 'jenis','sn','no_inventarisasi','tempat','bagian','tahun','tgl_kalibrasi','suplier','jenis_pengadaan','asal','harga','status','fungsi' );
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			else if ( $aColumns[$i] != ' ' )
			{
				if($i==0){
					...
					$pilih=mysql_fetch_assoc(mysql_query("select kode from logbook_h where kode='$kode'"));
					...
					case "logistik":
						...
						$link=$link."<a href=pmilogistik.php?module=edit_logbook&Kode=".$kode." TITLE=\"Edit data Barang\"><img src=\"images/ubah.png\" width=15 height=15/></a>";
					break;
					case "admin":
						$lokal=strpos(substr($kode,0,4),$utd[id]);
						//$link=$link."<a href=pmilogistik.php?module=lihat_minta&Kode=".$kode." TITLE=\"Lihat permintaan\"><img src=\"images/button_search.png\" width=15 height=15/></a> ";
