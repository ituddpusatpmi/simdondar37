...
if (!empty($_FILES["filecsv"]["tmp_name"])) {
	...
	$jml=0;
	?>
	<a name="atas"><h2>HASIL IMPORT HBSAG, File : <?=$_FILES['filecsv']['name']?></h2>
		...
		while (($data = fgetcsv($datacsv, 1000, $pemisah)) !== FALSE){
			...
			if (($gagalimport==0) and ($kontrolvalid==0)){
				if ($jml>11 and $data[0]<>"[End Of Results]" and strlen($data[1])>0 and $data[1]<>"Patient ID") {
					...
					$od			= round($od, 3);
					if ($ktg==0){
						$sql="insert into imltd_import_temp (tanggal,nokantong,hbsag_cut_off,hbsag_quanti,hbsag_reader,hbsag_result,hbsag_od,reagen_hbsag,hbsag_metode)
							    values('$tanggal','$data[1]','$cut_off','$data[3]','$data[4]','$data[5]','$od','$reag0_ex[0]','$reag0_ex[5]')";
						$tambah=mysql_query($sql,$con);
						$add++;
					}else{
						$sql=	"update imltd_import_temp set  tanggal = '$tanggal', hbsag_cut_off = '$cut_off', hbsag_quanti = '$data[3]', hbsag_reader = '$data[4]',
								hbsag_result	= '$data[5]', hbsag_od = '$od', reagen_hbsag = '$reag0_ex[0]', hbsag_metode = '$reag0_ex[5]' where nokantong = '$data[1]'";
						$tambah=mysql_query($sql,$con);
						$upd++;
					}
					?>
	...
	<a href="pmiimltd.php?module=import_etimax3000hcv"class="swn_button">Lanjut ke Anti-HCV</a>
...
<form action="" method="post" enctype="multipart/form-data" name="UploadCSV" id="UploadCSV">
	<h2>HBsAg</h2>
	<h2>LANGKAH IMPOR ke 1 dari 4 langkah</h2>
	<table border=0>
	<tr>
		<td>Pilih Reagen Diasorin/Murex HBsAg
		<select name="reagen0" id="reagen0" onChange="show0(0)" STYLE="width: 300px">
			<option value="">Pilih reagen Diasorin/Murex HBsAg</option>
				<? 
				$jreagen1=mysql_query("select * from reagen where ((Nama like '%Diasorin%HBsAg%') or (Nama like '%Murex%HBsAg%')) and aktif='1' and jumTest>0");
  ...
  <h2>Pilih file txt/csv untuk Parameter HbsAg</h2>
    <input type="file" name="filecsv" id="filecsv" class="swn_button" />
    <input type="submit" name="button" id="button" value="Proses Import HBsAg" class="swn_button"/>
</form>
<a href="pmiimltd.php?module=import_etimax3000hcv"class="swn_button">Lewatkan</a>
<a href="pmiimltd.php?module=import_etimax3000"class="swn_button">Kembali ke awal</a>
<? }?>
</body>
</html>
