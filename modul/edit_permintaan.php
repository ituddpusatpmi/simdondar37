...
if (file_exists($myfile)) { $fh=fopen($myfile,'a') or die ("Cant open file"); } else { $fh=fopen($myfile,'w') or die ("Cant open file"); }
	$update_os_sql="update dtranspermintaan set GolDarah ='$gol_darah', Rhesus ='$rhesus',JenisDarah='$jenisdarah' where NoForm='$NoForm'";
...
$myfile="bdrs/dari-$td0-$tgl_yesterday.zip";
if (file_exists($myfile)) { $fh=fopen($myfile,'a') or die ("Cant open file"); } else { $fh=fopen($myfile,'w') or die ("Cant open file"); }*/
	$update_os_sql1="update htranspermintaan set NamaOS='$namaos', golDrh='$golDrh',rhesus='$rhesus',jenis='$layanan',bagian='$bagian' where NoForm='$NoForm'";
...
/*
	...
	for ($i=0;$i<count($_POST['no_kantong']);$i++) {
		...
		if ($ckt==0) {
			$tambah_sql="insert into dtransaksipermintaan (`NoForm`, `NoKantong`, `Status`, `MetodeCross`,`StatusCross`,`stat2`, `Ket`, `aglutinasi`, `listcomb`,`tgl`,`petugas`,`cheker`,`mengesahkan`) 
				                               values ('$NoForm','$NoKantong','$titip', '$metode',    '$status',    '$cross','$ket','$aglutinasi','$listcomb','$tgl_permintaan','$sah2',' $sah1', '$sah')";
	...
	if ($tambah) {
		...
		?> <META http-equiv="refresh" content="2; url=pmilaboratorium.php?module=edit_datapasien&noform=<?=$NoForm?>"> <?
	}
	echo "Data tidak berhasil di entri ";
	$_POST['periksa']="";
}
if ($_POST['periksa']=="") { ?>
	...
	<table class="form" border=0 cellspacing=0 cellpadding=2>
		<tr>
			...
			<td>Nama OS</td>
			<td class="input"> <input type="text" name="namaos" value="<?=$check_h1[NamaOS]?>"></input></td>
		</tr>
		<tr>
			<td>Nama RS</font></td>
			<?
		...
		<tr>
			<td>Bagian</td>
			<td class="input"> <input type="text" name="bagian" value="<?=$check_h1[bagian]?>"></input></td>
			<td>Layanan</td>
			<td class="input"> <input type="text" name="layanan" value="<?=$check_h1[jenis]?>"></input></td>
</tr>
<tr>
<td>Gol. Darah Pasien</td>
		...
		<td>Jenis Komponen</font></td>
			<td class="input">
				...
				<option value="Leucodepleted" <?=$selected["Leucodepleted"]?>>Leucodepleted</option>
				</select>
			</td>
		...
		<!--tr>
			<td>No. Kantong</td>
			<td><input name="NoKantong" type="text" id="nokantong" 
			onkeydown="chang(event,this);" onchange="focus(document.periksa.NoKantong);cari_kantong('box-table-b');"/></td><td></td><td></td>
		</tr-->
	</table>
	
	<!--TABLE class="list" id="box-table-b">
		<tr class="field">
			<td>No Kantong</td><td>Tgl. Aftap</td><td>Gol. Darah</td><td>Metode Crossmatch</td><td>Status Crossmatch</td><td>Hasil Crossmatch</td><td>Ket</td><td>Aglutinasi</td><td>No Listcomb</td><td>Ket Keluar</td>
		</tr>
</TABLE>
<tr>
</tr>
...
<tr>
			<!--td colspan="9">Dicatat Oleh :  <?echo $petugas;?></td>

		<td colspan="9">Dikerjakan Oleh :
			<select name="sah2" > <?
				$user2="select * from user";
                $do2=mysql_query($user2);
					while($data2=mysql_fetch_assoc($do2)) {
						$select2=""; ?>
						<option value="<?=$data2[id_user]?>"<?=$select2?>>
						<?=$data2[id_user]?>
						</option>
				<? } ?>
			</select>
