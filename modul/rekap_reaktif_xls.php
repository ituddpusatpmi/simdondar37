...
$hasil=mysql_query("select noKantong as nk, jenis as jn, tglperiksa as tgl,gol_darah as gd,RhesusDrh as rh,tgl_Aftap as ta,statKonfirmasi as kon from stokkantong where hasil='4' and ident='m' and CAST(tglperiksa as date) >='$today' and CAST(tglperiksa as date) <='$today1' order by tglperiksa ASC");

$hbs=mysql_fetch_assoc(mysql_query("select count(jenisPeriksa and hasil) as hbs from hasilelisa where jenisPeriksa='0' and hasil='1' and CAST(tglPeriksa as date)>='$today' and CAST(tglPeriksa as date)<='$today1' "));

$hcv=mysql_fetch_assoc(mysql_query("select count(jenisPeriksa and hasil) as hcv from hasilelisa where jenisPeriksa='1' and hasil='1' and CAST(tglPeriksa as date)>='$today' and CAST(tglPeriksa as date)<='$today1' "));

$hiv=mysql_fetch_assoc(mysql_query("select count(jenisPeriksa and hasil) as hiv from hasilelisa where jenisPeriksa='2' and hasil='1' and CAST(tglPeriksa as date)>='$today' and CAST(tglPeriksa as date)<='$today1' "));

$syp=mysql_fetch_assoc(mysql_query("select count(jenisPeriksa and hasil) as syp from hasilelisa where jenisPeriksa='3' and hasil='1' and CAST(tglPeriksa as date)>='$today' and CAST(tglPeriksa as date)<='$today1' "));

	$TRec=mysql_num_rows($hasil);
?>
      <tr class="record">
        <th colspan=19><b>Total =
...
<td><?=$hasil14?>, <?=$reak14[OD]?></td>
        <?
$ptgs=mysql_fetch_assoc(mysql_query("select dicatatOleh from hasilelisa where nokantong='$baris[nk]'"));
$tgl=$reak[tglPeriksa];
