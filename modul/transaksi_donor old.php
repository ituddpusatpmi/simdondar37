...
if (isset($_POST['submit'])){
	...
	$tambah=mysql_query("insert into htransaksi 
			...
			JenisDonor,id_permintaan,Status,Nopol) 
	value ('$id_transaksi_baru','$id','$id1','-','$tmpt[0]','$_POST[instansi]','$today1',
		'$_POST[JenisDonor]','$_POST[id_permintaan]','0','$nopol')");
	...
	if ($tambah) {
		...
		if ($check_i>=1) {			
		$updatedonor=mysql_query("UPDATE pendonor SET instansi='$_POST[instansi]' WHERE Kode='$id'");
		}
		$instansi=mysql_query("select instansi from pendonor where Kode='$id'");
		$instansi1=mysql_fetch_assoc($instansi);
		if ($instansi1[instansi] <> $_POST[instansi]){
			$updatedonor2=mysql_query("UPDATE pendonor SET instansi='$_POST[instansi]' WHERE Kode='$id'");
		}
...
<table class="form" cellspacing="0" cellpadding="2">
	...
	//$check2=mysql_query("select * from pendonor where Kode_lama='$_GET[Kode_lama]'");
