		...
		for ($z=0;$z<96;$z++){
			...
			if ((strlen($sampleid[$nomor])!==0) and ($sampleid[$nomor]!=="NC1") and ($sampleid[$nomor]!=="PC1")){
				...
				if (($sampleid[$nomor]!=="") and ($threshold[$nomor]!=="*")){
					...
					if ($ktg==0){
						$sql="insert into imltd_import_temp (tanggal,    nokantong,          syp_cut_off, syp_reader,        syp_result,           syp_od,        reagen_syp,   syp_metode)
						...
						$sql=	"update imltd_import_temp set tanggal = '$tanggal', syp_cut_off = '$cut_off', syp_reader= '$odresult[$nomor]',
								syp_result	= '$hasiltmp', syp_od = '$ratio[$nomor]', reagen_syp = '$reag0_ex[0]',
								syp_metode	= '$reag0_ex[5]' WHERE nokantong = '$sampleid[$nomor]'";
						$tambah=mysql_query($sql,$con);
						$upd++;
					}
				}
...
<?
	...
	echo ("<SCRIPT LANGUAGE='JavaScript'>
		window.alert('File tidak bisa diimport. Bukan Parameter Syphilis. Pastikan file yang dipilih adalah parameter Syphilis!!!!')
...
<form action="" method="post" enctype="multipart/form-data" name="UploadCSV" id="UploadCSV">
	<font size=6 color="blue"><b>Parameter Trepanostika</b></font><br>
	<table class="list" border="1" cellpadding="2" cellspacing="1">
		<tr align="center" class="field">
			<td>Pilih Reagen Biomerieux Trepanostika
			<select name="reagen3" id="reagen3" onChange="show0(3)" STYLE="width: 300px">
				<option value="">Pilih reagen Biomerieux Trepanostika</option>
				<? 
				$jreagen1=mysql_query("select * from reagen where ((Nama like '%Biomerieux%SYP%') or (Nama like '%Biomerieux%Syp%')) and aktif='1' and jumTest>0");
			...
			<td><div id='kode3'></div></td>
			<td><div id='b3'></div></td>
	</tr>
	</table>
	<font size=4 color="black"><b>Pilih File CSV</b></font><br>
    <input type="file" name="filecsv" id="filecsv" class="swn_button_blue" />
    <input type="submit" name="button" id="button" value="Proses Import Syphilis" class="swn_button_blue"/>
</form>
<a href="pmiimltd.php?module=import_davinci"class="swn_button_blue">Kembali</a>
<? }?>
</body>
</html>
