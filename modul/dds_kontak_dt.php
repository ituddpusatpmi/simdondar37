...
session_start();
	...
	$aColumns = array( 'asaludd', 'nama', 'alamat', 'kelamin', 'tgllahir', 'golda', 'rhesus', 'status','nopiagam','cincin','tglterima','tglinput','petugas' );
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			else if ( $aColumns[$i] != ' ' )
			{
				if($i==0){
					...
					$pilih=mysql_fetch_assoc(mysql_query("select nama from dds where asaludd like '%$kode%'"));
					switch ($_SESSION[leveluser]){
					case "monev":
						$lokal=strpos(substr($kode,0,4),$utd[id]);
						$link=$link."<a href=pmimonev.php?module=edit_kontak&asaludd=".$kode." TITLE=\"Edit Data DDS\"><img src=\"images/ubah.png\" width=15 height=15/></a>";
					break;
					case "admin":
						$lokal=strpos(substr($kode,0,4),$utd[id]);
					...
					$link="??";
						...
						$row[]=$link."&nbsp"."<a href=pmi".$_SESSION[leveluser].".php?module=kontak_transaksi&asaludd=".$kode." TITLE=\"Lihat transaksi\">
										".$kode."</a>";
				        ;
				}else if($i==3){ //Jenis Kontak
