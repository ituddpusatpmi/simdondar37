...
if (isset($_POST['submit'])){
    ...
    $check_p=mysqli_num_rows(mysqli_query($dbi,"select KodePendonor from htransaksi where NoTrans = '$notrans' and KodePendonor='$kodep'"));
  echo "$notrans";
    if ($check_p==0) {
        ...
        $tambah=mysqli_query($dbi,"insert into htransaksi  (NoTrans,KodePendonor,KodePendonor_lama,Tgl,Pengambilan,ketBatal,tempat,Instansi,
                ...
                '$umur[GolDarah]','$umur[Rhesus]','$umur[Pekerjaan]','$umur[jumDonor]', '$donorbaru','$namauser','$jam_donor','$rs1')");
        if ($tambah) {
      ...
      $update = mysqli_query($dbi,"update htransaksi set JenisDonor='$_POST[JenisDonor]', id_permintaan='$_POST[id_permintaan]', apheresis='$donor_aph',donor_tpk='$donor_tpk' where NoTrans='$notrans'");
  }?>
    <META http-equiv="refresh" content="1; url=formulir_donor_PDF2.php?kp=<?=$id?>">
<?php };
$kode=$_GET['Kode'];
