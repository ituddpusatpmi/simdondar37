...
<font size="5" color=red>DATA PEMERIKSAAN NAT eSAS <b>Procleix<sup>&reg</sup> Ultrio Plus</b></font><br><br>
<?
...
<form name="cari" method="POST" action="<?echo $PHPSELF?>">
	...
	<?php
	$no=0;
	$sql="SELECT DATE(  `date_transfer` ) AS tgl,  trim(`run_number`) as run_number ,  `protocol` ,  `operator_name` ,  `userinput` ,  count(run_number) as jmltest, `konfirmasi`, `tgl_konfirmasi`, `userkonfirmasi`
		FROM  `imltd_procleix_raw`
		WHERE DATE(`date_transfer`)>='$tglawal' and DATE(`date_transfer`)<='$hariini'
		group by
		`run_number` ,(  `date_transfer` ),  `protocol` ,  `operator_name` ,  `userinput` ,`konfirmasi`,`tgl_konfirmasi`, `userkonfirmasi`";
	...
	while($tmp=mysql_fetch_assoc($qraw)){
		...
		<tr class="record">
			...
			<td align="left"><?=$tmp['tgl_konfirmasi']?></td>
			<td align="left"><?=$tmp['userkonfirmasi']?></td>
			<? if ($tmp['konfirmasi']==0){
				$konf="Belum";?>
				<td><a href="pmiimltd.php?module=import_nat_procleix_konfirm&no=<?=$tmp['run_number']?> "class="swn_btn_kiri">Konfirm</a>
			        <a href="pmiimltd.php?module=import_nat_procleix_rawdel&no=<?=$tmp['run_number']?> "class="swn_btn_tengah">Hapus</a>
					<a href="pmiimltd.php?module=import_nat_procleix_raw_result&no=<?=$tmp['run_number']?> "class="swn_btn_kanan">Lihat</a></td>
				<?}else{
	...
	if ($no==0){?>
		<tr class="record">
			<td colspan=10>Tidak ada data transfer pemeriksaan NAT Procleix<sup>&reg</sup> Ultrio Plus <br>dari tanggal <?=$tglawal?> sampai dengan tanggal <?=$hariini?></td>
	<?}?>
	</table><br>
