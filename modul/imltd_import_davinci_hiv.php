...
if (!empty($_FILES["filecsv"]["tmp_name"])) {
	...
	while (($data = fgetcsv($datacsv, 150, $pemisah)) !== FALSE){
		...
		if ($jml==7){
			...
			$kodeassay=substr($data[1], 0, 1);
		}
		if ($kodeassay!=="A"){break;}
...
if ($kodeassay=="A"){
	...
	<table class="list" border="1" cellpadding="2" cellspacing="1">
		...
		for ($z=0;$z<96;$z++){
			$nomor++;
			if ((strlen($sampleid[$nomor])!==0) and ($sampleid[$nomor]!=="NC1") and ($sampleid[$nomor]!=="PC1") and ($sampleid[$nomor]!=="PC2") and ($sampleid[$nomor]!=="PC3")){
				$tanggal=date("Y-m-d");
				...
				if (($sampleid[$nomor]!=="") and ($threshold[$nomor]!=="*")){
					...
					if ($ktg==0){
						$sql="insert into imltd_import_temp (tanggal,    nokantong,          hiv_cut_off, hiv_reader,        hiv_result,           hiv_od,        reagen_hiv,   hiv_metode)
						...
						$sql=	"update imltd_import_temp set tanggal = '$tanggal', hiv_cut_off = '$cut_off', hiv_reader= '$odresult[$nomor]',
								hiv_result	= '$hasiltmp', hiv_od = '$ratio[$nomor]', reagen_hiv = '$reag0_ex[0]',
								hiv_metode	= '$reag0_ex[5]' WHERE nokantong = '$sampleid[$nomor]'";
						$tambah=mysql_query($sql,$con);
						$upd++;
					}
				}
	...
	<a href="pmiimltd.php?module=import_davincisyp"class="swn_button_blue">Lanjut ke Syphilis</a>
...
<?
	...
	echo ("<SCRIPT LANGUAGE='JavaScript'>
		window.alert('File tidak bisa diimport. Bukan Parameter HIV. Pastikan file yang dipilih adalah parameter HIV!!!!')
...
<form action="" method="post" enctype="multipart/form-data" name="UploadCSV" id="UploadCSV">
	<font size=6 color="blue"><b>Parameter HIV</b></font><br>
	<table class="list" border="1" cellpadding="2" cellspacing="1">
		<tr align="center" class="field">
			<td>Pilih Reagen Biomerieux HIV
			<select name="reagen2" id="reagen2" onChange="show0(2)" STYLE="width: 300px">
				<option value="">Pilih reagen Biomerieux HIV</option>
				<? 
				$jreagen1=mysql_query("select * from reagen where ((Nama like '%Biomerieux%HIV%') or (Nama like '%Biomerieux%hiv%')) and aktif='1' and jumTest>0");
				while ($jreagen11=mysql_fetch_assoc($jreagen1)) { 
					$nr1=strtoupper($jreagen11[Nama]);
					$merk1=str_replace("HIV","",$nr1);
					...
					if ($merk11['nama_reagen']!='') {
						$m_reagen1=mysql_fetch_assoc(mysql_query("select reaktif,nonreaktif,greyzone from master_reagen 
										where nama_reagen='$merk11[nama_reagen]' and jenis_reagen='HIV'"));?>
			...
			<td><div id='kode2'></div></td>
			<td><div id='b2'></div></td>
	</tr>
	</table>
	<font size=4 color="black"><b>Pilih File CSV</b></font><br>
    <input type="file" name="filecsv" id="filecsv" class="swn_button_blue" />
    <input type="submit" name="button" id="button" value="Proses Import HIV" class="swn_button_blue"/>
</form>
<a href="pmiimltd.php?module=import_davincisyp"class="swn_button_blue">Lewatkan ke Syphilis</a>
<a href="pmiimltd.php?module=import_davinci"class="swn_button_blue">Kembali ke awal</a>
<? }?>
</body>
</html>
