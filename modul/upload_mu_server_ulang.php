...
//========================== UPDATE ======================================//
    ...
    $jml_donorulang=0;
    $jml_datadonormu=0;
    $pd=mysql_query("select * from pendonor where mu='3'",$con_local);
    while ($pd1=mysql_fetch_assoc($pd)) {
        ...
        if (mysql_num_rows($pdserver)=='1') {
            ...
            title,telp2,umur,jns,ketdarah,tglkembali,sukubangsa,cetak,ibukandung,mu,pencatat,up,waktu_update,tanggal_entry,p10,p25,p50,p75,p100,psatya,pprov,instansi)
                    VALUES
                ...
                '$namauser',b'1','$pd1[waktu_update]','$pd1[tanggal_entry]','$pd1[p10]','$pd1[p25]','$pd1[p50]','$pd1[p75]',
                '$pd1[p100]','$pd1[psatya]','$pd1[pprov]','$pd1[instansi]')";
    ...
    $jml_trxbaru=0;
    $jml_trxlama=0;
    $ht=mysql_query("select * from htransaksi where mu='3'",$con_local);
    ...
    //stok kantong
    $jml_kantong=0;
    $sk=mysql_query("select * from stokkantong where mu='3'",$con_local);
    while ($sk1=mysql_fetch_assoc($sk)) {
        ...
        $updateserver=mysql_query("UPDATE stokkantong SET
            ...
            kadaluwarsa='$sk1[kadaluwarsa]',RhesusDrh='$sk1[RhesusDrh]',gol_darah='$sk1[gol_darah]',produk='$sk1[produk]',tgl_Aftap='$sk1[tgl_Aftap]', lama_pengambilan='$sk1[lama_pengambilan]'
    ...
    //idcard
    $jml_idcard=0;
    $idc=mysql_query("select * from idcard where mu='3'",$con_local);
    ...
    $jml_batal=0;
    $jml_berhasil=0;
    $hasil =mysql_fetch_assoc(mysql_query("select count(distinct NoTrans) as jum  from htransaksi where mu='3' and Pengambilan='0'",$con_local));
    $hasil1=mysql_fetch_assoc(mysql_query("select count(distinct NoTrans) as jum1 from htransaksi where mu='3' and Pengambilan='1'",$con_local));
    $hasil2=mysql_fetch_assoc(mysql_query("select count(distinct NoTrans) as jum2 from htransaksi where mu='3' and Pengambilan='2'",$con_local));
    $hasil0=mysql_fetch_assoc(mysql_query("select count(distinct NoTrans) as jum from htransaksi where mu='3'",$con_local));
    $jml_gagal      = $hasil2['jum2'];
    $jml_batal      = $hasil1['jum1'];
    $jml_berhasil   = $hasil['jum'];
    $jml_transaksi  = $hasil0['jum'];
    $kg=mysql_query("select * from kegiatan where mu='3'",$con_local);
    while ($kg1=mysql_fetch_assoc($kg)) {
        $kgs=mysql_query("update kegiatan set TglPelaksanaan='$kg1[TglPelaksanaan]', mu='2',jamselesai='$tgl3',status='1', sukses='$hasil[jum]',batal='$hasil1[jum1]',gagal='$hasil2[jum2]' where NoTrans='$kg1[NoTrans]'");
    }
    $kg=mysql_query("select * from detailinstansi",$con_local);
    while ($kg1=mysql_fetch_assoc($kg)) {
        $kgs=mysql_query("update detailinstansi set nama='$kg1[nama]', alamat='$kg1[alamat]',telp='$kg1[telp]',cp='$kg1[cp]',
    ...
    $log_mdl ='MOBILE UNIT';
    $log_aksi='Upload Ulang Data Mobile Unit : '.$namainstansi;
    include "user_log.php";
    $log_aksi='Upload Ulang Data Pendonor Baru : '.$jml_donorbaru;
    include "user_log.php";
    $log_aksi='Upload Ulang Data Pendonor Ulang : '.$jml_donorulang;
    include "user_log.php";
    $log_aksi='Upload Ulang Data Transaksi Donor : '.$jml_transaksi.' (Berhasil : '.$jml_berhasil.'; Gagal : '.$jml_gagal.'; Batal : '.$jml_batal.')';
    include "user_log.php";
    $log_aksi='Upload Ulang Data Kantong :  '.$jml_kantong;
    include "user_log.php";
    //=====================================================================================================//Audit Trial

    //Upload Ulang Audit Trail
    //echo "<br> Audit trial ";
    ...
    ?>
    <div style="background-color: #ffffff;font-size:24px; color:#0099ff;text-shadow: 1px 1px 1px #000000; font-family:Verdana;">HASIL UPLOAD ULANG DATA MOBILE UNIT</div>
...
<?php
    ...
    <div style="background-color: #ffffff;font-size:24px; color:#0099ff;text-shadow: 1px 1px 1px #000000; font-family:Verdana;">UPLOAD ULANG DATA MOBILE UNIT KE SERVER</div>
    <div style="background-color: #ffffff;font-size:24px; color:#0099ff;text-shadow: 1px 1px 1px #000000; font-family:Verdana;">KODE LAPTOP: <?=$kodelaptop;?></div><br>
