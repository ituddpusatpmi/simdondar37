...
<script language=javascript src="js/dlkomponen_pool.js" type="text/javascript"> </script>
...
//        echo "Pembuatan Komponen berhasil.<br>";
        $ikomp=mysql_query("update stokkantong set Status='$dtA[Status]', produk='$njt',sah='$dtA[sah]',volume='$vlm',
			...
			tglpengolahan='$tglp1',tgl_Aftap='$tglaftap',kadaluwarsa='$nkd',statKonfirmasi='1',abs='negative',
			gol_darah='$gol', RhesusDrh='$rh', kodePendonor='$kodep' where noKantong='$nkt'");

	$ikomp2=mysql_query("update stokkantong set Status='6', produk='$njt',kadaluwarsa='$nkd' where noKantong='$nkta'");
        ...
        if($vlm!="None"){
            $hkom=mysql_query("INSERT INTO hpengolahan (notrans,user,nokantong,gd,rh,komponen,volume,musnah,tglpembuatan,pemisahan,Putar,Pisah,metode)
                   values
                   ('$id_transaksi_baru','$namauser','$nkt','$gol','$rh','$njt','$vlm','$musnah','$sekarang','$pisah','-','$mesin','2')");
        }
        //=======Audit Trial====================================================================================
        $log_mdl ='PENGOLAHAN';
        $log_aksi='Pengolahan Pooled : '.$nkt.' dari '.$nkta.', produk: '.$njt;
        ...
        $hkomp=mysql_query("select * from dpengolahan where noKantong='$nkt'");
            ...
            if (mysql_num_rows($hkomp)=='1') {
             $ukomp=mysql_query("UPDATE dpengolahan SET trans='$maxp',Produk='$njt', petugas='$namauser',metode='2',mulai='$mulai', selesai='$selesai', tgl='$tglp1', cara='$mesin',pisah='$pisah',gol_darah='$gol', RhesusDrh='$rh',jenis='$jenis[jenis]', where noKantong='$nkt',shift='$shift'");
        }else{
            $ikomp=mysql_query("insert into dpengolahan (trans,noKantong,Produk,petugas,tgl,cara,pisah,goldarah,rhesus,jenis,shift,metode,mulai,selesai,KantongAsal) value('$maxp','$nkt','$njt','$namauser','$tglp1','$mesin','$pisah', '$gol', '$rh','$jenis[jenis]','$shift','2','$mulai','$selesai','$nkta')");
        }
        if ($musnah=='1') {
...
?>
<h1 align="center">Pengolahan Komponen Metode Pool</h1>
<form name="komponen" id="komponen" onsubmit="return ok()" method="POST" action="<?=$PHPSELF?>">
	...
	<br>Masukkan No. Kantong Asal-->
    	<INPUT type="text" name="nokantong" id="nokantong" placeholder="input nokantong'A'-->" onkeydown="chang(event,this);" onchange="addRow('box-table-b')" /> Jika diketik manual tekan ENTER
    ...
    <TABLE class="form" id="box-table-b">
        <tr class="field">
            <th rowspan='2'>Kantong Pooled</th>
