			...
			$(document).ready(function() {
				...
				$('#searchBox').keypress(function(e) {
					if (e.which === 13) { // Tombol Enter
						...
						// console.log(keyword);
						oTable.fnFilter(keyword); // Gunakan filter manual
					}
				});
			});
...
<body id="dt_example" class="ex_highlight_row">
	...
	if ($td0 == 'M') {
		...
		if ($ninstan != 1) {
			$pesan = "SILAKAN <b><a href=pmimobile.php?module=data_jadwal_mobile>PILIH/GANTI INSTANSI</a></b> DULU SEBELUM MELANJUTKAN!!!";
		} else {
			$pesan = $instan01[nama];
		}
	...
	<div id="containerdt">
		<? if ($_SESSION[leveluser] == 'p2d2s') { ?>
			<h1 class="table">DATA PENDONOR <?= $saya ?>||
				<input type="button" value="Laporan Piagam" onClick="document.location.href='pmikasir.php?module=laporan_piagam';">
			</h1>
		<? } else { ?>
			...
			<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
				<thead>
					<tr class="ui-widget-header">
						<th>Cari : <input type="text" id="searchBox" placeholder="Ketik kemudian enter" /></th>
						<th></th>
						<th></th>
						<th></th>
						<th></th>
