...
//Notrans 	tanggal 	noform 	nama 	rs 	sifat 	status 	up 	jamtiba 	jamsls 	id_udd 
$sqlpasien="select * from daftarpasien where date(tanggal) >=current_date";
$no=0;
$sqlpasien1=mysql_query($sqlpasien,$con);
while ($sqlpasien2=mysql_fetch_assoc($sqlpasien1)) {
    $no++;
    $trans1[$no] 	= $sqlpasien2['Notrans'];
    $tanggal1[$no] 	= $sqlpasien2['tanggal'];
    $noform[$no]	= $sqlpasien2['noform'];
    $nama[$no]		= $sqlpasien2['nama'];
    $rs[$no]		= $sqlpasien2['rs'];
    $sifat[$no]		= $sqlpasien2['sifat'];
    $status[$no]	= $sqlpasien2['status'];
    $up[$no]		= $sqlpasien2['up'];
    $jamtiba[$no]	= $sqlpasien2['jamtiba'];
    $jamsls[$no]	= $sqlpasien2['jamsls'];
    $udd1[$no]		= $sqlpasien2['id_udd'];
}
...
for ($i=1;$i<count($namaproduk);$i++) {
	   ...
	   if ($chkserver[udd]==$id_udd) {$updserver=("UPDATE stokdarah SET nama='$namalengkap[$i]',a_pos  = '$Ap[$i]', b_pos  = '$Bp[$i]', o_pos  = '$Op[$i]', ab_pos = '$ABp[$i]', a_neg  = '$An[$i]',
		...
		$updateserver2=mysql_query("update `stok` as s, stokdarah as d set s.a_pos=(select sum(a_pos) from stokdarah) , s.b_pos=(select sum(b_pos) from stokdarah), s.o_pos=(select sum(o_pos) from stokdarah), s.ab_pos=(select sum(ab_pos) from stokdarah), s.a_neg=(select sum(a_neg) from stokdarah), s.b_neg=(select sum(b_neg) from stokdarah), s.o_neg=(select sum(o_neg) from stokdarah), s.ab_neg=(select sum(ab_neg) from stokdarah)  where s.udd=d.udd ");
		$addserver2=mysql_query($updateserver2,$con_pmipusat);
...
/*tanggal 	noform 	nama 	rs 	sifat 	status 	up 	jamtiba 	jamsls*/
for ($i=1;$i<count($trans1)+1;$i++){
		$chkserverp=mysql_fetch_assoc(mysql_query("select udd from daftarpasien where udd='$id_udd' and Notrans='$trans1[$i]' ", $con_pmipusat));
		if ($chkserverp[udd]==$id_udd){$updserverp="UPDATE daftarpasien set tanggal='$tanggal1[$i]', noform='$noform[$i]', nama='$nama[$i]', rs='$rs[$i]',
		sifat='$sifat[$i]', status='$status[$i]', up='$up[$i]', jamtiba='$jamtiba[$i]', jamsls='$jamsls[$i]', update_on=current_timestamp 
		WHERE Notrans='$trans1[$i]' AND udd='$udd1[$i]'";
		$updserver1p=mysql_query($updserverp,$con_pmipusat);
		}else{
	$addserverp="INSERT INTO daftarpasien (Notrans,tanggal, noform, nama, rs, sifat, status, up, jamtiba, jamsls,udd, update_on)
	VALUES ('$trans1[$i]','$tanggal1[$i]','$noform[$i]','$nama[$i]','$rs[$i]','$sifat[$i]','$status[$i]','$up[$i]','$jamtiba[$i]','$jamsls[$i]','$udd1[$i]', current_timestamp)";
	//	VALUES ('$NoTrans[$i]','$TglPenjadwalan[$i]','$instansi[$i]','$alamat[$i]','$jumlah[$i]','$lat[$i]','$lng[$i]','$kendaraan[$i]','12:00:00','13:00:00','$udd[$i]', current_timestamp)";			
		$addserver1p=mysql_query($addserverp,$con_pmipusat);}}
