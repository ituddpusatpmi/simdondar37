...
$ptg=mysql_query("SELECT `petugas` FROM htranspermintaan");if(!$ptg){mysql_query("ALTER TABLE `htranspermintaan` ADD `petugas` VARCHAR(7 ) DEFAULT NULL AFTER `nojenis`");}
$nat=mysql_query("SELECT `nat` FROM dtranspermintaan");if(!$nat){mysql_query("ALTER TABLE `dtranspermintaan` ADD `nat` CHAR(1 ) DEFAULT '0'");}
$wil=mysql_query("SELECT `wilayah` FROM htranspermintaan");if(!$wil){mysql_query("ALTER TABLE `htranspermintaan` ADD `wilayah` CHAR(1 ) DEFAULT '0'");}
//copy table htranspermintaan
mysql_query("create table if not exist copyhtranspermintaan as select * from htranspermintaan");
//hapus table htranspermintaan lama
$col4=mysql_query("SELECT `no_rm` FROM `htranspermintaan`");if(!$col4){mysql_query("drop table htranspermintaan");
...
$col5=mysql_query("SELECT `Nama` FROM `ruangan`");if(!$col5){mysql_query("CREATE TABLE `ruangan`(`Nama` varchar (30), PRIMARY KEY(`Nama`))");}


$col6=mysql_query("SELECT `nama` FROM `diagnosa`");if(!$col6){mysql_query(" CREATE TABLE `diagnosa` (
...
$col7=mysql_query("SELECT * FROM htranspermintaan where jenis_permintaan='0'");if($col7){mysql_query("update htranspermintaan set jenis_permintaan='Biasa' where jenis_permintaan='0'");}
$col8=mysql_query("SELECT * FROM htranspermintaan where jenis_permintaan='1'");if($col8){mysql_query("update htranspermintaan set jenis_permintaan='Cadangan' where jenis_permintaan='1'");}
$col9=mysql_query("SELECT * FROM htranspermintaan where jenis_permintaan='2'");if($col9){mysql_query("update htranspermintaan set jenis_permintaan='Siap Pakai' where jenis_permintaan='2'");}
$col10=mysql_query("SELECT * FROM htranspermintaan where jenis_permintaan='3'");if($col10){mysql_query("update htranspermintaan set jenis_permintaan='Cyto/Segera' where jenis_permintaan='3'");}


// update table dtranspermintaan
$col2=mysql_query("SELECT ID FROM dtranspermintaan");if (!$col2){mysql_query("ALTER TABLE `dtranspermintaan` ADD `ID` INT( 11 ) NOT NULL AUTO_INCREMENT FIRST , ADD PRIMARY KEY ( `ID` ) ");}
...
$pmb=mysql_query("SELECT `tempat` FROM `dpembayaranpermintaan`");if (!$pmb){mysql_query("ALTER table `dpembayaranpermintaan` ADD `tempat` VARCHAR( 7 ) NOT NULL DEFAULT 'UDD'");}
$pmb1=mysql_query("SELECT `no_rm` FROM `dtranspermintaan`");if (!$pmb1){mysql_query("ALTER TABLE `dtranspermintaan` ADD `no_rm` VARCHAR( 20 ) NOT NULL");}
$pmb2=mysql_query("SELECT `rs` FROM `dpembayaranpermintaan`");if (!$pmb2){mysql_query("ALTER table `dpembayaranpermintaan` 
...
$nojam=mysql_query("SELECT `nojaminan` FROM `htranspermintaan`");if (!$nojam){mysql_query("ALTER table `htranspermintaan` ADD `nojaminan` VARCHAR( 20 ) NULL DEFAULT '-'");}
...
if (isset($_POST[submit1])) {
	...
	$udd=mysql_fetch_assoc(mysql_query("select id from utd where aktif='1'"));
...
//------------------------ END set id transaksi ------------------------->
	...
	if ($cd=='0') {
		$permintaan="INSERT INTO `htranspermintaan` (`noform`, `bagian`, `kelas`, `namadokter`, `tglminta`, `diagnosa`, `alasan`, `hb`, `jenis`, `stat`, `rs`, `regrs`,`shift`, `tempat`, `nojenis`, `no_rm`, `umur`, `petugas`, `tgl_register`, `ruangan`, `pernah_transfusi`,`kapan`, `jenis_permintaan`, `reaksi_transfusi`, `gejala`, `jml_kehamilan`, `abortus`, `ket`,`nojaminan`)
				...
				'$_POST[reaksitransfusi]','$_POST[gejala]','$_POST[jmlkehamilan]','$_POST[abortus]','$_POST[ket]','$no_jaminan')";

		//insert table daftar pasien 
//tanggal 	noform 	nama 	rs 	sifat 	status 	up 	jamtiba 	jamsls
