...
$array_bulan = array(1=>'Jan','Feb','Mar', 'Apr', 'Mei', 'Jun','Jul','Ags','Sep','Okt', 'Nov','Des');
	 ...
	 */
	$aColumns = array( 'Kode', 'Nama', 'Alamat', 'kelurahan','kecamatan','wilayah','GolDarah', 'Rhesus', 'Jk', 'TglLhr','Cekal','jumDonor','tglkembali','Kode_lama','apheresis' );
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			else if ( $aColumns[$i] != ' ' )
			{
				if($i==0){
					...
					$data=mysql_fetch_assoc(mysql_query("select tglkembali,Cekal, apheresis from pendonor where Kode = '$kode' "));
		...
		case "mobile":
				...
				$link=$link."<a href=pmimobile.php?module=transaksi_donor&Kode=".$kode." TITLE=\"Transaksi Donor\"><img src=\"images/bloodbag.png\" width=20 height=15/></a> ";
				}
		...
		case "kasir":
				$lokal=strpos(substr($kode,0,4),$utd[id]);
				if ($antri[KodePendonor]!=$kode and $data[Cekal]=="0" and strtotime($data[tglkembali])<=strtotime($today)) {
				$link=$link."<a href=pmikasir.php?module=transaksi_donor&Kode=".$kode."&apheresis='0' TITLE=\"Transaksi Donor\"><img src=\"images/bloodbag.png\" width=20 height=15/></a> ";
                                if ($data[apheresis]=='1'){
                                    $link=$link."<a href=pmikasir.php?module=transaksi_donor&Kode=".$kode."&apheresis='1' TITLE=\"Transaksi Donor Apheresis\"><img src=\"images/apheresis.png\" width=20 height=15/></a> ";
                                }
				}
		...
		case "bdrs":
				$lokal=strpos(substr($kode,0,4),$utd[id]);
				if ($antri[KodePendonor]!=$kode and $data[Cekal]=="0" and strtotime($data[tglkembali])<=strtotime($today)) {
				$link=$link. "<a href=pmibdrs.php?module=transaksi_donor&Kode=".$kode." TITLE=\"Transaksi Donor\"><img src=\"images/bloodbag.png\" width=20 height=15/></a>  ";
				}
