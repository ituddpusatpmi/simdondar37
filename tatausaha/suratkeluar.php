...
<script type="text/javascript">
  jQuery(document).ready(function(){	
  	$('#kelurahan').autocomplete({source:'modul/suggest_tu_tujuan.php', minLength:2});});
</script>
<script type="text/javascript">
  jQuery(document).ready(function(){
...
if (isset($_POST[submit])) {
	...
	*/
	...
	$sekarang = date("Y-m-d h:m:s");
	 ...
	 $tambah_sql="insert into suratkeluar 
	...
	$kelurahan=mysql_query("insert into tu_tujuan (tujuan) values ('$kelurahan')");
	$kecamatan=mysql_query("insert into tu_perihal (perihal) values ('$kecamatan')");
	//echo $tambah_sql;	
	...
	if ($tambah) {
		  ...
		  switch ($lv0){
			   case "tatausaha":
					?>
				<META http-equiv="refresh" content="0; url=pmitatausaha.php?module=suratkeluar">
					<?
			   break;
			  /*case "p2d2s":
					?>
...
switch ($lv0) {
	 case "tatausaha":
		  ?><form name="reg" autocomplete="off" method="post" action="pmitatausaha.php?module=suratkeluar"><?
	 break;
	/*case "p2d2s":
...
<h1 class="table">FORM INPUT SURAT KELUAR ||
<input type="button" value="SEARCH SURAT KELUAR" onClick="document.location.href='pmitatausaha.php?module=search_suratkeluar';"></h1>
<? } ?>
	 ...
	 <tr>
		  <td>
			   ...
			   <tr>
					<td>Tempat Lahir</td>
					<td class="input">
						 <input name="tempatlhr" type="text" size="30" required placeholder="Kabupaten">
					</td>
			   </tr>
			   <tr> 
					<td>Tgl Lahir</td>
