...
<body>
	<font size="4" color=00008B>DATA PEMERIKSAAN HISCL 5000 BELUM KONFIRMASI</b></font><br><br>
	<style>
	...
	while($tmp=mysql_fetch_assoc($qraw)){
		...
		<tr style="font-size:13px; color:#000000; font-family:Verdana;" onMouseOver="this.className='highlight'" onMouseOut="this.className='normal'">
			...
			<td>
				<a href="pmiimltd.php?module=sysmexkonfirmasi&ins=<?=$tmp['instrument_name']?>&tgl=<?=$tmp['tanggal']?>&usr=<?=$tmp['operator']?>" class="swn_kiri">Konfirmasi</a> |
				<a href="pmiimltd.php?module=sysmexprocess&op=del&ins=<?=$tmp['instrument_name']?>&tgl=<?=$tmp['tanggal']?>&usr=<?=$tmp['operator']?>" onclick="return confirm('PERHATIAN \n \nYakin akan menghapus data transfer alat \n<?=$tmp['instrument_name']?>, \nTanggal: <?=$tmp['tanggal']?>, \nOperator: <?=$tmp['operator']?>, \nJumlah tes: <?=$tmp['jml_sample']?> ?');" class="swn_kanan">Hapus</a>
				
		</tr>
	<?}
	if ($no==0){?>
		<tr class="record">
			<td colspan=8>Data Sudah Diproses</td>
	<?}?>
	</table><br>
