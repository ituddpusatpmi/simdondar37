...
if (isset($_POST['simpan'])) { 
    ...
    for ($i=0; $i<sizeof($_POST['no']); $i++) { 
        ...
        $v_tipe = $_POST['tipe'][$i];
        $v_golongan = $_POST['golongan'][$i];
        $v_rhesus = $_POST['rhesus'][$i];
        $inst="INSERT INTO `samplekode`(`sk_trans`, `sk_tgl`, `sk_jenis`, `sk_kode`,`sk_gol`, `sk_rh`, `sk_user`) VALUES ('$notransaksi','$hariini','$v_tipe','$v_sample','$v_golongan','$v_rhesus','$namauser')";
        //echo $inst.'<br>';
        $inst=mysqli_query($dbi,$inst);
        if ($inst){echo $v_sample.' - OK<br>';}else{echo $v_sample.' - Gagal<br>';}
    }
    //=======Audit Trail====================================================================================
    $log_mdl ='LOGISTIK';
	...
	<form name="kantong" onsubmit="return ok()" method="POST" action="<?=$PHPSELF?>">
		<table cellpadding=1 cellspacing="0" style="min-width:700px;" class="bayangan">
			<tr style="background-color:mistyrose; font-size:12px; color:#000000;">
                ...
                $g1=$g2=$g3=$g4='';
                switch($v_gol){
                    case '1':$g1="selected";break;
                    case '2':$g2="selected";break;
                    case '3':$g3="selected";break;
                    case '4':$g4="selected";break;
                }
                ?>
                <td>Gol :<select name="gol">
						<option value="1" <?php echo $g1;?>>A</option>
						<option value="2" <?php echo $g2;?>>B</option>
						<option value="3" <?php echo $g3;?>>O</option>
                        <option value="4" <?php echo $g4;?>>AB</option>
				</select></td>
                <?php
                $r1=$r2="";
                switch($v_rh){
                    case '1' : $r1="selected";break;
                    case '2' : $r2="selected";break;
                }
                ?>
                <td>Rh :<select name="rh">
						<option value="1" <?php echo $r1;?>>+</option>
						<option value="2" <?php echo $r2;?>>-</option>
				</select></td>
	...
	<table id="box-table-b" style="background-color:#FECCBF; font-size:14px; color:#000000; font-family:arial;border-collapse:collapse;min-width:700px;" class="bayangan">
        ...
        for ($i=0; $i<$v_jml; $i++){
            ...
            <tr style="font-size:12px;height:30px; color:#000000; font-family:Verdana;" onMouseOver="this.className='highlight'" onMouseOut="this.className='normal'">
                    ...
                    <td><?php echo $gol.$rh;?><input type="hidden" name="golongan[]" value="<?php echo $gol;?>">
                                              <input type="hidden" name="rhesus[]" value="<?php echo $rh;?>"></td>
