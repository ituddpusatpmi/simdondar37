...
<div style="font-size:18px;color:#00008B;"><center><H2> <b>REKAP PEMERIKSAAN DARAH LENGKAP<br>LULUS PEMERIKSAAN</b></H2></center></div>
      ...
      <br>
        ...
        $previous = $halaman - 1;
        $next = $halaman + 1;
        $sql1="SELECT * from v_sample_selesai WHERE sk_hasil='1' order by sk_tgl_plebotomi ASC";
      ...
      if ($namadonor==''){
        $sql="SELECT * from  v_sample_selesai WHERE sk_hasil='1'  order by sk_tgl_plebotomi ASC limit $halaman_awal, $batas";
      }else{
           $sql="SELECT * from  v_sample_selesai WHERE sk_hasil='1' AND Nama like '%$namadonor%' order by sk_tgl_plebotomi ASC limit $halaman_awal, $batas";
      }
...
<nav>
    <ul class="pagination justify-content-center">
        <li class="page-item">
              <a class="page-link" <?php if($halaman > 1){ echo "href='?module=testlulus&halaman=$Previous'"; } ?>>Previous</a>
    </li>
    <?php
    for($x=1;$x<=$total_halaman;$x++){
    ?>
        <li class="page-item"><a class="page-link" href="?module=testlulus&halaman=<?php echo $x ?>"><?php echo $x; ?></a></li>
    <?php
    }
    ?>
        <li class="page-item">
        <a  class="page-link" <?php if($halaman < $total_halaman) { echo "href='?module=testlulus&halaman=$next'"; } ?>>Next</a>
        </li>
    </ul>
</nav>
