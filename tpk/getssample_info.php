...
$syarattpk='2';
$syarataph='2';
    $chk_sample=mysqli_num_rows(mysqli_query($dbi,"SELECT * FROM `samplekode` WHERE `sk_kode`='$kode_sample'"));
    if ($chk_sample>0){
        $samplevalid="1";
        $sqlu="SELECT `jenisPeriksa`, `Hasil`, `noKantong`,`tglPeriksa`, `idsample` FROM `hasilelisa` WHERE `noKantong`='$kode_sample' or `idsample`='$kode_sample' and (`noKantong`=`idsample`) ORDER BY `tglPeriksa` DESC";
        ...
        $qnat=mysqli_fetch_array(mysqli_query($dbi,"SELECT *,date(tglPeriksa) as tgl FROM `hasilnat` WHERE `idsample`='$kode_sample' ORDER BY `tglPeriksa` DESC"));
        if ($qnat>1){
            $tgl_nat=$qnat['tgl'];
            switch($qnat['Hasil']){
        ...
        $sqlkgd="SELECT `GolDarah`,`Rhesus`,`Cocok`,date(tgl) as tgl FROM `dkonfirmasi` WHERE  `idsample`='$kode_sample' OR `NoKantong`='$kode_sample' and  (`NoKantong`=`idsample`) ORDER BY `id` DESC LIMIT 1";
        ...
        $qtiter=mysqli_fetch_array(mysqli_query($dbi,"SELECT * FROM `covid` WHERE `cov_sampel`='$kode_sample' ORDER BY `cov_tgl` DESC LIMIT 1"));
        if ($qtiter>1){
            $titer='0';
        ...
        $qhema=mysqli_fetch_array(mysqli_query($dbi,"SELECT `dl_id`, `dl_trx`, `dl_kantong`, `dl_sampel`, `dl_tgl`, `dl_labperiksa`, `dl_pj_lab`, `dl_hb`, `dl_hct`, 
                `dl_plt`, `dl_leu`, `on_insert`, `user_input` FROM `hematologi` WHERE `dl_sampel`='$kode_sample'"));
        if($qhema){
            $dl='0';
            $hem_hb     = $qhema['dl_hb'];
        ...
        $sqlabs="SELECT date(`abs_tgl`) as tgl, `abs_result` as `hasil` FROM `abs` WHERE  `abs_kode_sample`='$kode_sample' ORDER BY `abs_id` DESC LIMIT 1";
...
$sample_info=$hint_imltd.';'.$hint_nat.';'.$hint_kgd.';'.$hint_titer.';'.$hem_hb.';'.$hem_hct.';'.$hem_plt.';'.$hem_leu.';'.$hint_abs.';'.$samplevalid.';'.$syarataph.';'.$syarattpk.';'.$ujs.';'.$nat;
?> 