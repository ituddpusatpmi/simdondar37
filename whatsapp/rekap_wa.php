...
$sms=mysql_query("select wa_time as jam,wa_no as hp,wa_text from wagw.sent where wa_no !='' and DATE_FORMAT(wa_time, '%Y %m %d' ) = DATE_FORMAT( CURDATE( ) , '%Y %m %d')  order by jam ASC");

if (isset($_POST[submit])) { $sms=mysql_query("select wa_time as jam,wa_no as hp,wa_text from wagw.sent where wa_no !='' and DATE(wa_time) between '$_POST[tgl]' and '$_POST[tgl1]' order by jam ASC");

}
$jml=$sms;
?>
<h1>Jumlah Rekap Pesan Terkirim = <?=mysql_num_rows($jml)?>  Pesan</h1>
...
<th>No.</th><th>Jam</th><th>HP</th><th>Pesan</th></tr>
<?
...
<?
echo "<td>$no</td><td>$sms1[jam]</td></td><td>$sms1[hp]</td><td>$sms1[wa_text]</td></tr>";
