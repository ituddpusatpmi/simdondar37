	<head>
		...
		$(function() {
			$(document).ready(function() {
				...
				"sPaginationType": "full_numbers",
					...
					"sAjaxSource": "whatsapp/wa_outbox_dt.php",
					"oLanguage": {
		...
		function hapus(id){
			//console.log(id);
			$.ajax({
				url: "modul/sms_outbox_json.php?id="+id,
			    async: false,
			    dataType: 'json',
...
<?
require_once("config/db_connect_wa.php");
if (isset($_POST[submit])) {
    $empty=mysql_query("TRUNCATE TABLE outbox");
    if($empty){
	echo '<META http-equiv="refresh" content="0; url=pmiadmin.php?module=wa_outbox">';
    }else{
        echo mysql_error();
    }
}
...
<body id="dt_example" class="ex_highlight_row">
	<div>
		<div id="dynamic">
		<br>
        <h2> Daftar Outbox Pesan </h2>
        <form name="empty" method="post" action="pmiadmin.php?module=kosongkan_wa_outbox">
