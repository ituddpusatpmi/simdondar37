...
echo "<a href=pmiadmin.php?module=wa_broadcast>BACK</a>";
...
<input type=submit name=submit2 value='Kirim Pesan ke Semua Pendonor ini!'>
";
echo "<table>";
...
if ($_POST[instansi]!='') $search.=" AND instansi like '%$_POST[instansi]%' ";
...
echo " ";
$pd=mysql_query("select Kode, nama,GolDarah,rhesus,alamat,instansi,Jk,Status,telp2,concat(DATE_FORMAT(tglkembali,'%e '),ELT( MONTH(tglkembali), 'Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'),DATE_FORMAT(tglkembali,' %Y')) as tgl from pendonor $search GROUP BY Kode ");
$no=0;
...
if (isset($_POST[submit2])) {
    echo "Mengirimkan Whatsapp broadcast Donor Kembali......";
