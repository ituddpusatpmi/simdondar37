...
if (isset($_POST['terima'])) {
    for ($i=0; $i<count($_POST[golfix]); $i++) {
        ...
        $sampeltpk=mysql_query("UPDATE samplekode set sk_gol='$goldarah', sk_rh='$rhesus' where sk_kode='$nokt' ");
        $tambah4=mysql_query("UPDATE konfirmasi_temp set stat='1' WHERE NoKantong='$nokt'");
        $tambah21="insert into dkonfirmasi(NoKonfirmasi,NoKantong,GolDarah,Rhesus,tgl,petugas,Cocok,goldarah_asal,
    ...
    //=====================================================================================================
    if ($tambah2) {
        echo "<br><b>Golongan Darah ABO dan Rhesus Telah berhasil diupdate";
  ...
  onMouseOver="this.className='highlight'" onMouseOut="this.className='normal'" >
    <?
        ...
        $ambil =     "SELECT \n".
"                konfirmasi_temp.tgl, \n".
"                samplekode.sk_kode as nokt,                 \n".
"                samplekode.sk_gol as gollama,                  \n".
"                samplekode.sk_rh as rhlama, \n".
...
"                FROM \n".
"                konfirmasi_temp \n".
"                INNER JOIN samplekode ON konfirmasi_temp.NoKantong = samplekode.sk_kode \n".
"              WHERE konfirmasi_temp.stat=0 \n".
"                GROUP BY konfirmasi_temp.NoKantong";
?>
<?

    $ambilreag1   = mysql_query($ambil);
    $numr         = mysql_num_rows($ambil);
        $ambilreagen  =    mysql_fetch_assoc($ambilreag1);
?>
                    <tr >
...
<table>
        ...
        <tr>
                ...
                <td class=input width=150px >
                <input type="hidden" name=nokt[] value=<?=$temp[nokt]?>>
                <input type="hidden" name=nokantong[] value=<?=$temp[nokt]?>><?=$temp[nokt]?></td>
                ...
                <td class=input><input type="hidden" name=kodep[] value=<?=$temp[sk_donor]?>><?=$temp[sk_donor]?></td>
