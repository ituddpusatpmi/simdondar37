...
<body>
	...
	?>
	<font size="4" color=00008B>DATA KONFIRMASI PEMERIKSAAN Architect i2000SR</b></font><br><br>
	...
	$no=0;
    $jmlsample=0;
	$sql="SELECT `no_trans`,`instr`, `arc_serial`,  date(`koonfirm_time`) as tanggal, `konfirmer`, count(`id_tes`) as jumlah 
		 FROM `imltd_arc_konfirm` WHERE  DATE(`koonfirm_time`)>='$tglawal' and DATE(`koonfirm_time`)<='$hariini'
		 group by `no_trans`, `instr`, `arc_serial`, date(`koonfirm_time`), `konfirmer`";
	$qraw=mysql_query($sql);
	while($tmp=mysql_fetch_assoc($qraw)){
		$no++;
        $jmlsample=$jmlsample+$tmp['jumlah'];
		?>
		<tr style="font-size:13px; color:#000000; font-family:Verdana;" onMouseOver="this.className='highlight'" onMouseOut="this.className='normal'">
			...
			<td>
				<a href="pmiimltd.php?module=imltd_arc2000_view&notrans=<?=$tmp['no_trans']?> ">Lihat</a>
		</tr>
	<?}
	if ($no==0){?>
        <tr style="font-size:13px; color:#000000; font-family:Verdana;" onMouseOver="this.className='highlight'" onMouseOut="this.className='normal'">
			<td colspan=8 align="center">Tidak ada data konfirmasi pemeriksaan IMLTD Arcitect <i>i2000SR</i></td>
         </tr>
	<?} else{
        ?>
        <tr class="field">
            <td align="center" colspan="5">JUMLAH SAMPLE</td>
            <td align="right"><?=$jmlsample?></td>
            <td></td>
        </tr>
    <?}?>
