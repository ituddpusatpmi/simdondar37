...
if(isset($_POST['Button']))  {
	...
	$reag2_nama=$reag2_ex[0]; 	$reag2_nolot=$reag2_ex[2]; 	$reag2_kode=$reag2_ex[2];	$reag2_tes=$reag2_ex[3];    $reag2_ed=$reag2_ex[4];
	...
	if ($lanjut == 0){
		...
		for ($i=0;$i<count($_POST[sample]);$i++) {
            ...
            if ($idraw_b=="0"){$koder_b=null;$edr_b=null;}else{$koder_b=$reag1_kode;$edr_b=$reag1_ed;}
            if ($idraw_c=="0"){$koder_c=null;$edr_c=null;}else{$koder_c=$reag2_kode;$edr_c=$reag2_ed;}
            if ($idraw_i=="0"){$koder_i=null;$edr_i=null;}else{$koder_i=$reag3_kode;$edr_i=$reag3_ed;}
            if ($idraw_s=="0"){$koder_s=null;$edr_s=null;}else{$koder_s=$reag4_kode;$edr_s=$reag4_ed;}
            $catatan="";
...
<table border=0 cellpadding="5" cellspacing="5" width="100%">
   <tr>
		<td align="left" style="background-color: #ffffff"><font size=5 color="blue"><b>KONFIRMASI HASIL PEMERIKSAAN ABBOTT ARCHITECT <i>i</i>2000SR</b></font></td>
...
<form name="manual_input" align="left" method="post" action="<?echo $PHPSELF?>">
	...
	<tr class="record">
		...
		<td colspan=4><?=$row['user'];$userarc=$row['user'];?>		<input type=hidden name=user       value="<?=$row['user']?>"></td>
	...
	<table class="list" border=1 cellpadding="2" cellspacing="2" width="100%" style="border-collapse:collapse">
		...
		<tr style="background-color:#FF6346; font-size:14px; color:#FFFFFF; font-family:Verdana;" onMouseOver="this.className='highlight'" onMouseOut="this.className='normal'">
			<td align="center">OD</td><td align="center">Lot</td><td align="center">Hasil</td><td align="center">Ket</td>
			<td align="center">OD</td><td align="center">Lot</td><td align="center">Hasil</td><td align="center">Ket</td>
			<td align="center">OD</td><td align="center">Lot</td><td align="center">Hasil</td><td align="center">Ket</td>
			<td align="center">OD</td><td align="center">Lot</td><td align="center">Hasil</td><td align="center">Ket</td>
		</tr>
		...
		while($data=mysql_fetch_assoc($sq_sample)){
			...
			//Clear Var
                ...
                $id_raw_i='0';
                $id_raw_s='0';
				$hbsag_value='';    $hbsag_lotr='';     $hbsag_label='';    $hbsag_hasil='';    $hbsag_v='';    $sketperiksa1='';   $hbsag_snr='';  $rtime_b='';;
				$hcv_value='';      $hcv_lotr='';       $hcv_label='';      $hcv_hasil='';      $hcv_v='';      $sketperiksa2='';   $hcv_snr='';    $rtime_c='';
				$hiv_value='';      $hiv_lotr='';       $hiv_label='';      $hiv_hasil='';      $hiv_v='';      $sketperiksa3='';   $hiv_snr='';    $rtime_i='';
				$trep_value='';     $trep_lotr='';      $trep_label='';     $trep_hasil='';     $trep_v='';     $sketperiksa4='';   $trep_snr='';   $rtime_s='';
			...
			while ($row2=mysql_fetch_assoc($row)){
				...
				switch ($row2['parameter']){
                    case 'HBsAgQ2' : $hbsag_value =$row2['abs'];
                                     $hbsag_lotr  =$row2['lot_reag'];
									 $hbsag_label =$row2['hasil'];
                                     $hbsag_snr   =$row2['sn_reag'];
									 if ($row2['hasil']=='Nonreactive') {$hbsag_hasil='0';$hbsag_v='NR';}
                                     elseif ($row2['hasil']=='Grayzone'){$hbsag_hasil='2';$hbsag_v='GZ';$cekal='1';}
									 elseif ($row2['hasil']=='Reactive'){$hbsag_hasil='1';$hbsag_v='R';$cekal='1';}
									 ...
									 break;
					case 'SyphilisTP':
					case 'Syphilis': $trep_value=$row2['abs'];
                                     $trep_label=$row2['hasil'];
                                     $trep_lotr=$row2['lot_reag'];
                                     $trep_snr=$row2['sn_reag'];
									 if ($row2['hasil']=="Nonreactive") {$trep_hasil='0';$trep_v='NR';}
									 elseif ($row2['hasil']=="Grayzone") {$trep_hasil='2';$trep_v='GZ';$cekal='4';}
									 elseif ($row2['hasil']=="Reactive") {$trep_hasil='1';$trep_v='R';$cekal='4';}
                                     ...
                                     $jml_test_s++;
									 break;
					case 'Anti-HCV': $hcv_value=$row2['abs'];
									 $hcv_label=$row2['hasil'];
                                     $hcv_lotr=$row2['lot_reag'];
                                     $hcv_snr=$row2['sn_reag'];
									 if ($row2['hasil']=='Nonreactive'){$hcv_hasil='0';$hcv_v='NR';}
                                     elseif ($row2['hasil']=="Grayzone"){$hcv_hasil='2';$hcv_v='GZ';$cekal='2';}
                                     elseif ($row2['hasil']=="Reactive"){$hcv_hasil='1';$hcv_v='R';$cekal='2';}
                                     ...
                                     $jml_test_c++;
									 break;
					case '_HIV Ag/Ab': $hiv_value=$row2['abs'];
                                     $hiv_label=$row2['hasil'];
                                     $hiv_lotr=$row2['lot_reag'];
                                     $hiv_snr=$row2['sn_reag'];
									 if ($row2['hasil']=='Nonreactive'){$hiv_hasil='0';$hiv_v='NR';}
                                     elseif ($row2['hasil']=="Grayzone"){;$hiv_hasil='2';$hiv_v='GZ';$cekal='3';}
                                     elseif ($row2['hasil']=="Reactive"){;$hiv_hasil='1';$hiv_v='R';$cekal='3';}
			...
			$nr_all="0";
			if ((strlen($hbsag_label)>0) and (strlen($hcv_label)>0) and (strlen($hiv_label)>0) and (strlen($trep_label)>0)){$all_param='1';} else {$all_param='0';}
			?>
			<tr style="font-size:13px; color:#000000; font-family:Verdana;" onMouseOver="this.className='highlight';" onMouseOut="this.className='normal';">
				...
				<? if($hbsag_label=="Reactive" or $hbsag_label=="Grayzone"){ ?>
					<td align='center'> 	<input type="hidden" name=hbsag_result[] value=<?=$hbsag_hasil?>> <font color="red"><b>  <?=$hbsag_v;?> </b></font></td>
					<? $nr_all="1";?><input type="hidden" name=cekal_hbsag[] value="1"><?
					} else { ?>
					<td align='center'> 	<input type="hidden" name=hbsag_result[] value=<?=$hbsag_hasil?>> <font color="black">  <?=$hbsag_v;?> </font></td>
				...
				<? if($hcv_label=="Reactive" or $hcv_label=="Grayzone"){ ?>
					<td align='center'> 	<input type="hidden" name=hcv_result[] value=<?=$hcv_hasil?>> <font color="red"><b>  <?=$hcv_v;?> </b></font></td>
					<? $nr_all="2";?><input type="hidden" name=cekal_hcv[] value="1"><?
					} else { ?>
					<td align='center'> 	<input type="hidden" name=hcv_result[] value=<?=$hcv_hasil?>> <font color="black">  <?=$hcv_v;?> </font></td>
				...
				<? if($hiv_label=="Reactive" or $hiv_label=="Grayzone"){ ?>
					<td align='center'> 	<input type="hidden" name=hiv_result[] value=<?=$hiv_hasil?>> <font color="red"><b>  <?=$hiv_v;?> </b></font></td>
					<? $nr_all="3";?><input type="hidden" name=cekal_hiv[] value="1"><?
					} else { ?>
					<td align='center'> 	<input type="hidden" name=hiv_result[] value=<?=$hiv_hasil?>> <font color="black">  <?=$hiv_v;?> </font></td>
				...
				<? if($trep_label=="Reactive" or $trep_label=="Grayzone"){ ?>
					<td align='center'> 	<input type="hidden" name=syp_result[] value=<?=$trep_hasil?>> <font color="red"><b>  <?=$trep_v;?> </b></font></td>
					<? $nr_all="4";?><input type="hidden" name=cekal_syp[] value="1"><?
					} else { ?>
					<td align='center'> 	<input type="hidden" name=syp_result[] value=<?=$trep_hasil?>> <font color="black">  <?=$trep_v;?> </font></td>
