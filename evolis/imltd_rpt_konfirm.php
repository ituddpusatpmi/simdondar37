...
class MYPDF extends TCPDF {
    ...
    $sql = mysqli_query($dbi,"SELECT `id`, `no_trans`, `instr`, `trans_time`, `user`,
		 `id_tes`,
...
EOD;
			...
			while ($isi = mysqli_fetch_array($sql)) {
                ...
                $opr_arc=mysqli_fetch_assoc(mysqli_query($dbi,"select nama_lengkap from user where id_user='$operator'"));
                if ($opr_arc){$operator_arc=$opr_arc[nama_lengkap];} else {$operator_arc=$operator;}
                $konfr=mysqli_fetch_assoc(mysqli_query($dbi,"select nama_lengkap from user where id_user='$konfirmer'"));
                if ($konfr){$konfirmer_arc=$konfr[nama_lengkap];}else{$konfirmer_arc=$konfirmer;}
                $sah_arc=mysqli_fetch_assoc(mysqli_query($dbi,"select nama_lengkap from user where id_user='$pengesah'"));
    	        ...
    	        <tr><td colspan="3" align="left">Operator Alinity</td>
