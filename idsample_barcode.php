...
// PRINT VARIOUS 1D BARCODES
    ...
    $ck=10;
    $hpengolahan=mysql_query("select * from samplekode where sk_trans='$notrans'");
    while($holah=mysql_fetch_assoc($hpengolahan)){

        if ($holah['sk_jenis']=="APH"){
            $jenis = "APHERESIS";
        }else if ($holah['sk_jenis']=="TPK"){
...
$pdf->SetX(2);
$pdf->write1DBarcode($holah['sk_kode'], 'C128', '', '', 44, 13.5, '', $style, 'T');
$pdf->SetX(56);
$pdf->write1DBarcode($holah['sk_kode'], 'C128', '', '', 42, 13.5, '', $style, 'N');

$pdf->SetFont('helveticaB', '', 8.5);
$pdf->SetXY(1,13.5);
$pdf->Cell(0, 0,$jenis,0, 0,'L');
