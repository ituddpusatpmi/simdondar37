...
function clearForm() {
	document.getElementById('id_sample11').value="";
	document.getElementById('id_sample11').focus();
}
...
function check() {
    valid_darah0 = '0';
    var no_kantong = document.periksa.id_sample11.value;
	no_kantong=no_kantong.toUpperCase();
        $.ajax({
            url: "json_ceksample.php?NoKantong="+no_kantong,
            async: false,
            dataType: 'json',
	...
	if (valid_darah0=='0'){
		clearForm();
                $( "#Sample_tdk_sesuai" ).dialog( "open" );
     
    }
}

$(function() {
		$( "#sample_tdk_sesuai" ).dialog({
            autoOpen: false,
			height: 200,
			modal: true,
            hide: "explode"
