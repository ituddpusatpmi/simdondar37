...
<h2 class="list">REKAP RINCIAN BARANG MASUK</h2>
...
?>
	...
	<table class="list" cellpadding=5 cellspacing=1>
		...
		while ($data1 = mysql_fetch_assoc($data)) {
			$no++;
			$jumlah = number_format($data1['qtymasuk'], 0, ',', '.');
		...
		?>
			...
			<tr class="record">
				...
				switch ($data1[jenis]) {
					case "BB":
						echo "<td align=center><a href=pmilogistik.php?module=rincian_transaksi_bantuan&notrans=$data1[notrans]>$data1[notrans]</a></td>";
						break;
					case "PJ":
						echo "<td align=center><a href=pmilogistik.php?module=rincian_transaksi_beli&notrans=$data1[notrans]>$data1[notrans]</a></td>";
						break;
					default:
						echo "<td align=center>$data1[notrans]</td>";
				}
				?>
