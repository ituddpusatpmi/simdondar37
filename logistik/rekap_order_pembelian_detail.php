...
<h2 class="list">REKAP ORDER PEMBELIAN BARANG</h2>
...
<?
    $data=mysql_query("SELECT
                      *,
                      ...
                      FROM
                      pmi.hstok_order_detail
                      JOIN pmi.hstok
                      ON pmi.hstok_order_detail.kode = pmi.hstok.kode
                      WHERE
                      hstok_order_detail.qtyorder >= 1 AND hstok_order_detail.harga > 1
                      AND date(hstok_order_detail.insert_on) between '$_POST[waktu]' AND '$_POST[waktu1]'
                      GROUP BY
                      hstok_order_detail.kode");
                      
                      
    
?>
<table class="list" cellpadding=5 cellspacing=1 width=80%>
    <tr class="field">
        <td>No</td>
        <td>No. Order</td>
        <td>Nama Barang</td>
    ...
    <tr class="record">
        ...
        <td align="left"><a href=pmilogistik.php?module=rincian_order_beli&notrans=<?=$data1[notrans]?>><?=$data1[notrans]?></a></td>
        <td align="left"><?=$data1[nama]?></td>
        <td align="left"><?=$data1[satuan]?></td>
        <?
            $detail=mysql_fetch_assoc(mysql_query("select sum(qtyorder) as qtymasuk, harga, sum(subtotal) as subtotal
                                                  from hstok_order_detail where kode='$data1[kode]'  and date(insert_on) between '$_POST[waktu]' AND '$_POST[waktu1]'"));
